...
bool CSaveGameDialog::ParseSaveData( char const *pszFileName, char const* pszShortName, KeyValues *kv )
{
	...
	FileHandle_t fh = filesystem()->Open( pszFileName, "rb", NULL );
...
void CSaveGameDialog::ScanSavedGames( void )
{
	...
	char const *pFileName = filesystem()->FindFirst( szDirectory, &handle, NULL );
	while (pFileName)
	{
...
const char *CSaveGameDialog::FindSaveSlot()
{
	...
	for (int i = 0; i < 1000; i++)
	{
		...
		FileHandle_t fp = filesystem()->Open( szFileName, "rb", NULL );
		if (!fp)
		{
			return szFileName + 5;
		}
