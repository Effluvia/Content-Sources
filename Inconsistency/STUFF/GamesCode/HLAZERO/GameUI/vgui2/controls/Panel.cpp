...
void Panel::Init(int x,int y,int wide,int tall)
{
	...
	///////////////////////////////////
	//MODDD - !!!!  Not now.  (original line here)
	ipanel()->Init(_vpanel, this);
	
	// EXPAND?
	//vgui::IPanel* thisInta = ipanel();
	//((VPanel *)_vpanel)->Init(this);
	//VPanel* somePano = (VPanel*)_vpanel;
	//somePano->Init(this);
	///////////////////////////////////
...
//-----------------------------------------------------------------------------
const char *Panel::LOCAL_GetClassName()
{
...
void Panel::PaintTraverse(bool repaint, bool allowForce)
{
	...
	// So you're the piece a shit that draws a square at the bottom-left corner of the screen.
	// What's this supposed to do.
	int pannerTest = GetVPanel();
...
/*
//MODDD - NEW.  ???
bool Panel::IsProportional(void){
	return false;
}
*/
/*
bool Panel::IsAutoDeleteSet(void){
	return false;
}
*/
void Panel::DeletePanel(void){
...
//MODDD - new params, new logic.  Get from decomp sometime
// Nah just go ahead and use these with the new param for now, just passes it along anyway
...
bool Panel::RequestFocusPrev(VPANEL panel)
{
	...
	*/
	...
	if (GetParent())
	{
		return GetParent()->RequestFocusPrev(panel);
	}
	return false;
}
...
bool Panel::RequestFocusNext(VPANEL panel)
{
	...
	*/
	if (GetParent())
	{
		return GetParent()->RequestFocusNext(panel);
	}
	return false;
}
...
void Panel::PostActionSignal( KeyValues *message )
{
	...
	for (i = _actionSignalTargetDar.GetCount() - 1; i > 0; i--)
	{
		...
		if (panel)
		{
			ivgui()->PostMessage(panel, message->MakeCopy(), GetVPanel(), 0);
		}
	}
	...
	if (i == 0)
	{
		...
		if (panel)
		{
			ivgui()->PostMessage(panel, message, GetVPanel(), 0);
		}
	}
	else
	{
...
void Panel::GetSettings( KeyValues *outResourceData )
{
	...
	outResourceData->SetString( "ControlName", LOCAL_GetClassName() );
...
PanelMessageMap* Panel::GetMessageMap(){
	/*
	if ((char)s_pMap != '\0') {
		return s_pMap;
	}
