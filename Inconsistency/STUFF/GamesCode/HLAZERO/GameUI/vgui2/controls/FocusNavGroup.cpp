	...
	if (best)
	{
        ...
        if (!best->CanBeDefaultButton())
        {
			...
			else
			{
				...
				{
					ivgui()->PostMessage(_mainPanel->GetParent()->GetVPanel(), new KeyValues("FindDefaultButton"), NULL, 0);
				}
			}
        }
        else
...
void FocusNavGroup::SetCurrentDefaultButton(Panel *panel)
{
	...
	{
		ivgui()->PostMessage(_currentDefaultButton->GetVPanel(), new KeyValues("SetAsCurrentDefaultButton", "state", 0), NULL, 0);
	}
	...
	if (_currentDefaultButton != NULL && _currentDefaultButton.Get())
	{
		ivgui()->PostMessage(_currentDefaultButton->GetVPanel(), new KeyValues("SetAsCurrentDefaultButton", "state", 1), NULL, 0);
	}
}
