...
public:
	...
	virtual void ReloadLocalizationFiles(IFileSystem *filesystem);
	virtual void ConstructString(wchar_t *unicodeOutput,int unicodeBufferSizeInBytes,char *tokenName,KeyValues *localizationVariables);
...
bool CLocalizedStringTable::AddFile(IFileSystem *fileSystem, const char *szFileName)
{
	...
	// parse out the file
	FileHandle_t file = fileSystem->Open(fileName, "rb", NULL);
	if (!file)
	{
		...
		if (langptr && !bTriedEnglish)
		{
			...
			file = fileSystem->Open(fileName, "rb", NULL);
		}
...
{
	// parse out the file
	FileHandle_t file = fileSystem->Open(szFileName, "wb", NULL);
	if (!file)
		return false;
...
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput,int unicodeBufferSizeInBytes,char *tokenName,KeyValues *localizationVariables){
