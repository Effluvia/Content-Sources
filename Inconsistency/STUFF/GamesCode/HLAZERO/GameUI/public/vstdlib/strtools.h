...
//MODDD - TEST?  Don't really know how this was intended to work,
// complains about missing "_Q_strcpy" at runtime if compiled as a DLL 
// (lib is still what's sent off to lib/public), and complains at compile time
// while compiling the 'vgui2' project if compiled as a static library.
// DONT. KNOW.
// (also, swap the _USRDLL costant for _LIB and vice versa if changing
// from compiling dynamic to static, see project properties -> General ->
...
VSTDLIB_INTERFACE const char*	_Q_strstr( const char* file, int line, const char *s1, const char *search );
