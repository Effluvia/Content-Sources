...
COptionsSubMultiplayer::COptionsSubMultiplayer(vgui::Panel *parent) : vgui::PropertyPage(parent, "OptionsSubMultiplayer") 
{
	...
	//MODDD - LAZY.  For now, sending blank strings for the now present 'text' parameter (3rd) other places used.
	// Wait!  Sending 'ABC' to make it obvious what it is
	m_pModelList = new CLabeledCommandComboBox( this, "Player model", "ABC" );
	InitModelList( m_pModelList );

	m_pLogoList = new CLabeledCommandComboBox( this, "SpraypaintList", "ABC" );
    m_LogoName[0] = 0;
	InitLogoList( m_pLogoList );

	m_pColorList = new CLabeledCommandComboBox( this, "SpraypaintColor", "ABC" );
...
/*	// cleanup old remap files
	...
	*/
	...
	char const *fn = filesystem()->FindFirst( directory, &fh, NULL );
...
void COptionsSubMultiplayer::RemapPalette( char *filename, int topcolor, int bottomcolor )
{
	...
	file = filesystem()->Open( infile, "rb", NULL );
	...
	file = filesystem()->Open( outfile, "wb", NULL );
	if ( file != FILESYSTEM_INVALID_HANDLE )
	{
