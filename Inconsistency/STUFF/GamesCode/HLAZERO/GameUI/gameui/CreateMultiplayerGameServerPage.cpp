...
void CCreateMultiplayerGameServerPage::LoadMapList()
{
	...
	const char *filename = filesystem()->FindFirst("maps/*.bsp", &findHandle, NULL);
	while (filename)
	{
		// remove the text 'maps/' and '.bsp' from the file name to get the map name
		char mapname[256];
		
		//MODDD - 'strstr' returns a 'const char*' now, adjustments made
		...
		if (str)
		{
			//MODDD - now that str is const, must be done differently
			//*str = 0;
			int relativeTo = str - mapname;
			mapname[relativeTo] = 0;
		}
