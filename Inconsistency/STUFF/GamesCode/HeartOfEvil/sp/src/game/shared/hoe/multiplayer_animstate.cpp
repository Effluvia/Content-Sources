...
#undef PLAYER_DOD // HOE_DLL DoD model test
...
#endif
{
	...
	m_bIsOnBarnacle = false;
	m_bFirstBarnacleFrame = false;
#endif
...
#ifdef HOE_THIRDPERSON
	case PLAYERANIMEVENT_RESET_GESTURE_SLOT:
		// This is needed to clear out PLAYERANIMEVENT_ATTACK_PRE if the weapon is switched for example.
		ResetGestureSlot( nData );
#ifdef HOE_SWITCH_GESTURE_WITH_MAIN_SEQUENCE
		if ( nData == GESTURE_SLOT_ATTACK_AND_RELOAD )
			KillTransitionGesture( GESTURE_SLOT_ATTACK_AND_RELOAD );
#endif
		break;
	case PLAYERANIMEVENT_GRENADE1_DRAW:
		RestartGesture( GESTURE_SLOT_ATTACK_AND_RELOAD, ACT_MP_GRENADE1_DRAW, false );
		break;
	case PLAYERANIMEVENT_GRENADE2_DRAW:
		RestartGesture( GESTURE_SLOT_ATTACK_AND_RELOAD, ACT_MP_GRENADE2_DRAW, false );
		break;
	case PLAYERANIMEVENT_GRENADE1_THROW:
		RestartGesture( GESTURE_SLOT_ATTACK_AND_RELOAD, ACT_MP_GRENADE1_ATTACK );
		break;
	case PLAYERANIMEVENT_GRENADE2_THROW:
		RestartGesture( GESTURE_SLOT_ATTACK_AND_RELOAD, ACT_MP_GRENADE2_ATTACK );
		break;
	case PLAYERANIMEVENT_BARNACLE_GRAB:
		m_bIsOnBarnacle = true;
		m_bFirstBarnacleFrame = true;
		RestartMainSequence();
		break;
	case PLAYERANIMEVENT_BARNACLE_RELEASE:
		m_bIsOnBarnacle = false;
		RestartMainSequence();
		break;
#endif
...
#ifdef HOE_SWITCH_GESTURE_WITH_MAIN_SEQUENCE
	if ( iGestureSlot == GESTURE_SLOT_ATTACK_AND_RELOAD )
		KillTransitionGesture( GESTURE_SLOT_ATTACK_AND_RELOAD );
#endif
}
...
bool CMultiPlayerAnimState::HandleMoving( Activity &idealActivity )
{
	...
	if ( flSpeed > MOVING_MINIMUM_SPEED )
	{
		...
		idealActivity = ( pWpn != NULL && ( GetBasePlayer()->m_fThirdPersonAimMode
			...
			|| !pWpn->HasIronSights() // chainsaw, machete
#endif
			)) ?
			ACT_RUN_AIM : ( pWpn ? ACT_RUN_RIFLE : ACT_RUN );
#else
		// Always assume a run.
		idealActivity = ACT_RUN_AIM;
#endif
	}
...
#ifdef HOE_THIRDPERSON
//-----------------------------------------------------------------------------
bool CMultiPlayerAnimState::HandleLadder( Activity &idealActivity )
{
...
//-----------------------------------------------------------------------------
bool CMultiPlayerAnimState::HandleBarnacle( Activity &idealActivity )
{
	if ( m_bIsOnBarnacle )
	{
		if ( m_bFirstBarnacleFrame )
		{
			RestartMainSequence();
			idealActivity = ACT_BARNACLE_HIT;
			m_bFirstBarnacleFrame = false;
		}
		else if ( (GetBasePlayer()->GetSequenceActivity( GetBasePlayer()->GetSequence() ) == ACT_BARNACLE_HIT) &&
			!GetBasePlayer()->IsSequenceFinished() )
		{
			idealActivity = ACT_BARNACLE_HIT; // continue playing this until done
		}
		else
		{
			idealActivity = ACT_BARNACLE_PULL;
		}
		return true;
	}
...
#ifdef HOE_THIRDPERSON
	CBaseCombatWeapon *pWpn = GetBasePlayer()->GetActiveWeapon();
	Activity idealActivity = ( pWpn != NULL
		&& (GetBasePlayer()->m_fThirdPersonAimMode
#ifdef HOE_IRONSIGHTS
		|| !pWpn->HasIronSights() // chainsaw, machete
#endif
		)
		) ? ACT_IDLE_ANGRY : ACT_IDLE;
#else // HOE_THIRDPERSON
	Activity idealActivity = ACT_IDLE_ANGRY;
#endif // HOE_THIRDPERSON

	if ( HandleJumping( idealActivity ) || 
#ifdef HOE_THIRDPERSON
		HandleBarnacle( idealActivity ) ||
		HandleLadder( idealActivity ) ||
#endif
...
#ifdef HOE_SWITCH_GESTURE_WITH_MAIN_SEQUENCE
	SwitchGestureWithMainSequence( GESTURE_SLOT_ATTACK_AND_RELOAD );
#endif
...
#ifdef HOE_SWITCH_GESTURE_WITH_MAIN_SEQUENCE
		if ( iGesture == GESTURE_SLOT_ATTACK_AND_RELOAD_TRANSITION )
			MaintainGestureTransition( GESTURE_SLOT_ATTACK_AND_RELOAD );
#endif
	}
}
...
//	Anim_StateLog( "-------------%s: frame %d -----------------\n", bIsServer ? "Server" : "Client", gpGlobals->framecount );
	...
	for ( int iAnim = 0; iAnim < GetBasePlayer()->GetNumAnimOverlays(); ++iAnim )
	{
		...
		{
			Anim_StatePrintf( iLine++, "Layer #%d %s: Weight: %.2f, Cycle: %.2f", iAnim, GetSequenceName( GetBasePlayer()->GetModelPtr(), pLayer->m_nSequence ), (float)pLayer->m_flWeight, (float)pLayer->m_flCycle );
			if ( m_aGestureSlots[iAnim].m_bActive == false ) DevWarning( "ACTIVE SEQUENCE (order = %d) INACTIVE GESTURE-SLOT #%d %s\n", pLayer->m_nOrder, iAnim, "client" );
		}
#else
...
#ifdef HOE_SWITCH_GESTURE_WITH_MAIN_SEQUENCE
void CMultiPlayerAnimState::SwitchGestureWithMainSequence( int iGestureSlot )
{
	GestureSlot_t *pGesture = &m_aGestureSlots[iGestureSlot];
	if ( !pGesture->m_bActive )
		return;
	...
	// Make sure we have a valid animation layer to fill out.
	if ( !pGesture->m_pAnimLayer )
		return;

	Activity iGestureActivity = TranslateActivity( pGesture->m_iActivity );
	if ( iGestureActivity != pGesture->m_iActivity )
	{
		CBasePlayer *pPlayer = GetBasePlayer();
		if ( !pPlayer )
			return;
		...
		char *cs = "server";
#endif
		DevMsg( "Swapping gesture sequence %s -> %s (%s)\n",
			GetSequenceName( pPlayer->GetModelPtr(), pGesture->m_pAnimLayer->m_nSequence ),
			GetSequenceName( pPlayer->GetModelPtr(), iGestureSequence ), cs );

		// Move the current gesture to the transition layer
		GestureSlot_t *pGestureTransition = &m_aGestureSlots[iGestureSlot+7];
		AddToGestureSlot( iGestureSlot+7, pGesture->m_iActivity, pGesture->m_bAutoKill );
		pGestureTransition->m_pAnimLayer->m_flCycle = pGesture->m_pAnimLayer->m_flCycle;
		pGestureTransition->m_pAnimLayer->m_flPrevCycle = pGesture->m_pAnimLayer->m_flPrevCycle;
		pGestureTransition->m_pAnimLayer->m_flLayerAnimtime = gpGlobals->curtime;
		pGestureTransition->m_pAnimLayer->m_flLayerFadeOuttime = 0.2;

		pGesture->m_iActivity = iGestureActivity;
		pGesture->m_pAnimLayer->m_nSequence = iGestureSequence;
#ifndef CLIENT_DLL
		pGesture->m_pAnimLayer->m_nActivity = iGestureActivity;
#endif
	}
}

void CMultiPlayerAnimState::MaintainGestureTransition( int iGestureSlot )
{
	GestureSlot_t *pGesture = &m_aGestureSlots[iGestureSlot];
//	if ( !pGesture->m_bActive )
//		return;
	...
	GestureSlot_t *pGestureTransition = &m_aGestureSlots[iGestureSlot+7];
	if ( !pGestureTransition->m_bActive )
		return;
	...
	// If the layer we are shadowing goes away, the transition layer goes away as well
	if ( !pGesture->m_bActive )
	{
		ResetGestureSlot( iGestureSlot+7 );
#ifndef CLIENT_DLL
		pPlayer->FastRemoveLayer( iGestureSlot+7 );
#endif
		return;
	}

	// See if the 0.2 seconds have elapsed (if so the transition layer weight is now zero)
	if ( gpGlobals->curtime >= pGestureTransition->m_pAnimLayer->m_flLayerAnimtime + pGestureTransition->m_pAnimLayer->m_flLayerFadeOuttime )
	{
		pGesture->m_pAnimLayer->m_flWeight = 1.0f;
		ResetGestureSlot( iGestureSlot+7 );
#ifndef CLIENT_DLL
		pPlayer->FastRemoveLayer( iGestureSlot+7 );
#endif
	...
	float weight = pGestureTransition->m_pAnimLayer->GetFadeout( gpGlobals->curtime );
	pGesture->m_pAnimLayer->m_flWeight = 1.0f - weight;
	pGestureTransition->m_pAnimLayer->m_flWeight = weight;
}

void CMultiPlayerAnimState::KillTransitionGesture( int iGestureSlot )
{
	GestureSlot_t *pGestureTransition = &m_aGestureSlots[iGestureSlot+7];
	if ( !pGestureTransition->m_bActive )
		return;
	...
	char *cs = "server";
#endif
	DevMsg( "Killing transition gesture sequence %s (%s)\n",
		GetSequenceName( pPlayer->GetModelPtr(), pGestureTransition->m_pAnimLayer->m_nSequence ), cs );
...
#endif // HOE_SWITCH_GESTURE_WITH_MAIN_SEQUENCE
