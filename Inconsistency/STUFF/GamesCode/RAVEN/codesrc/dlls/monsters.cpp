...
//
//=========================================================
BOOL CBaseMonster :: IsAngryAt ( CBaseEntity* pEntity )
{
	for(int i = 0; i < MAX_OLD_ENEMIES; i++)
	{
		if(m_hOldEnemy[i] == pEntity)
			return TRUE;
	}
	...
	if ( !FBitSet( pev->spawnflags, SF_MONSTER_PRISONER ) )
	{
		...
		for ( int i = 0; i < count; i++ )
		{
				 ...
				 pSightEnt->pev->health > 0 )
			{
				...
				if ( (IRelationship( pSightEnt ) != R_NO || IsAngryAt( pSightEnt ))
...
void CBaseMonster::ReportAIState( void )
{
	...
	while (1)
	{
		activity_map_t* pactivity = (activity_map_t*)GetActivityAtIndex(i);
		if(!pactivity->type)
			break;

		if ( pactivity->type == (int)m_Activity )
		{
			ALERT( level, "Activity %s, ", pactivity->name );
			break;
		}
		i++;
	}
