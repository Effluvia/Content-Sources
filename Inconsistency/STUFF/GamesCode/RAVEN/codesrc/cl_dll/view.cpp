...
extern void Game_UpdateScopeBeam( void );
...
==================
*/
void V_SetProperFOV( void )
{
	if(!cl_default_fov)
	{
		cl_default_fov = gEngfuncs.pfnGetCvarPointer( "default_fov" );
		if(!cl_default_fov)
			return;
	}
	
	float flAspect = (float)ScreenWidth/(float)ScreenHeight;
	float flFovX = ScaleFOVByWidthRatio( cl_game_fov->value, flAspect * 0.75f );

	cl_default_fov->value = flFovX;
}
...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	if(view && view->model && view->model->type != mod_alias)
	{
...
void DLLEXPORT V_CalcRefdef( struct ref_params_s *pparams )
{
	...
	if ( gModelCamera.IsActive() )
	{
		gModelCamera.CalcRefDef( pparams );
	}
	else if ( pparams->intermission )
	{	
	...
	V_SetProperFOV();
	R_SetFrustum(pparams->vieworg, pparams->viewangles, pparams->viewsize);
...
void V_Init (void)
{
	...
	cl_game_fov			= gEngfuncs.pfnRegisterVariable( "r_game_fov","90", 0 );
}

/*
=============
V_DrawAliasViewModel
=============
*/
void V_DrawAliasViewModel ( void )
{
	cl_entity_t* pview = gEngfuncs.GetViewModel();
	if(!pview || !pview->model || pview->model->type != mod_alias)
		return;

	glDepthRange(0.0, 0.1);
	gAliasRenderer.DrawAliasModel(pview);
	glDepthRange(0.0, 1.0);

	// don't let the engine draw the alias model, as the renderer
	// code is broken and doesn't process them properly
	pview->model = NULL;
}
