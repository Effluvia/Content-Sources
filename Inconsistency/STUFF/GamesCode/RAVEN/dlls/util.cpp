...
void Con_Printf( char *fmt, ... )
{
	...
	ALERT(at_console, "%s", cMsg);
}

void UTIL_VectorTransform (const vec3_t& in1, float in2[3][4], vec3_t& out)
{
...
void UTIL_AngleMatrix (const vec3_t& angles, float (*matrix)[4] )
{
	float		angle;
...
void UTIL_VectorScale (const vec3_t& in, vec_t scale, vec3_t& out)
{
	out[0] = in[0]*scale;
	out[1] = in[1]*scale;
	out[2] = in[2]*scale;
...
void UTIL_VectorAdd (const vec3_t& veca, const vec3_t& vecb, vec3_t& out)
{
	out[0] = veca[0]+vecb[0];
	out[1] = veca[1]+vecb[1];
	out[2] = veca[2]+vecb[2];
