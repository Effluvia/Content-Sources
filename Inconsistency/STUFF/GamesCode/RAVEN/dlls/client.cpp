...
void ClientCommand( edict_t *pEntity )
{
	...
	else if (strstr(pcmd, "fire"))
	{
		if(CMD_ARGC() > 1)
			FireTargets(CMD_ARGV(1), CBaseEntity::Instance(pev), CBaseEntity::Instance(pev), USE_TOGGLE, 0);
		else
			ALERT(at_console, "usage: fire <target>\n");
	}
...
void ClientPrecache( void )
{
	...
	PRECACHE_SOUND("player/pl_snow1.wav");		// breathe bubbles
...
Vector GetViewOrigin( edict_t* pViewEntity, edict_t* pClient )
{
	edict_t* pView = pViewEntity;
	if(!pView)
	{
		CBasePlayer* pPlayer = (CBasePlayer*)CBaseEntity::Instance( pClient );
		pView = pPlayer->GetViewEntity();
		if(pView)
		{
			int iAttachment = pPlayer->GetAttachment();

			Vector vOrigin, vAngles;
			g_engfuncs.pfnGetAttachment( pView, iAttachment, vOrigin, vAngles );
	...
	Vector vOrigin = pView->v.origin + pView->v.view_ofs;
	if ( pView->v.flags & FL_DUCKING )
		vOrigin = vOrigin + ( VEC_HULL_MIN - VEC_DUCK_HULL_MIN );
...
{
	Vector vOrigin = GetViewOrigin( pViewEntity, pClient );

	*pvs = ENGINE_SET_PVS ( (float *)&vOrigin );
	*pas = ENGINE_SET_PAS ( (float *)&vOrigin );
}
