...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "item_usedkevlar" );
	...
	// crowbar
	UTIL_PrecacheOtherWeapon( "weapon_crowbar_hl" );
...
void CBasePlayerItem::DefaultTouch( CBaseEntity *pOther )
{
	...
	if (pOther->AddPlayerItem( this ))
	{
		...
		if (!FClassnameIs(pev, "weapon_intro") && !FClassnameIs(pev, "weapon_kick"))
...
#endif
	{
		m_pPlayer->m_pBeforeAction = m_pPlayer->m_pActiveItem;

		if (m_pPlayer->pev->weapons & (1 << WEAPON_KICK))
			m_pPlayer->SwitchWeapon(m_pPlayer->m_rgpPlayerItems[16]);
	}
}
...
#endif
	{
		if (pOther->GiveAmmo(AMMO_GLOCKCLIP_GIVE * 4, "9mm", _9MM_MAX_CARRY) != -1 ||
			pOther->GiveAmmo(AMMO_DEVASTATORCLIP_GIVE * 2, "devastator_rockets", DEVASTATOR_MAX_CARRY) != -1 ||
			pOther->GiveAmmo(AMMO_RPGCLIP_GIVE * 2, "rockets", ROCKET_MAX_CARRY) != -1 ||
			pOther->GiveAmmo(AMMO_BUCKSHOTBOX_GIVE, "buckshot", BUCKSHOT_MAX_CARRY) != -1 ||
			pOther->GiveAmmo(MP5_MAX_CLIP * 2, "chaingun", _CHAINGUN_MAX_CARRY) != -1 ||
			pOther->GiveAmmo(5, "Satchel Charge", SATCHEL_MAX_CARRY) != -1 ||
			pOther->GiveAmmo(5, "Trip Mine", TRIPMINE_MAX_CARRY) != -1)
		{
