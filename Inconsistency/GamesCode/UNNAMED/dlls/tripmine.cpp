...
enum tripmine_old_e {
	OLDTRIPMINE_WORLD = 7,
	OLDTRIPMINE_GROUND = 8
};


#ifndef CLIENT_DLL

class CTripmineGrenadeOLD : public CGrenade
{
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(monster_tripmine, CTripmineGrenadeOLD);

TYPEDESCRIPTION	CTripmineGrenadeOLD::m_SaveData[] =
{
	DEFINE_FIELD(CTripmineGrenadeOLD, m_flPowerUp, FIELD_TIME),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_vecDir, FIELD_VECTOR),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_vecEnd, FIELD_POSITION_VECTOR),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_flBeamLength, FIELD_FLOAT),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_hOwner, FIELD_EHANDLE),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_pBeam, FIELD_CLASSPTR),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_posOwner, FIELD_POSITION_VECTOR),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_angleOwner, FIELD_VECTOR),
	DEFINE_FIELD(CTripmineGrenadeOLD, m_pRealOwner, FIELD_EDICT),
};

IMPLEMENT_SAVERESTORE(CTripmineGrenadeOLD, CGrenade);


void CTripmineGrenadeOLD::Spawn(void)
{
	Precache();
	// motor
	...
	SET_MODEL(ENT(pev), "models/v_tripmine_old.mdl");
	pev->frame = 0;
	pev->body = 3;
	pev->sequence = OLDTRIPMINE_WORLD;
	ResetSequenceInfo();
	pev->framerate = 0;
	...
	SetThink(&CTripmineGrenadeOLD::PowerupThink);
...
void CTripmineGrenadeOLD::Precache(void)
{
	PRECACHE_MODEL("models/v_tripmine_old.mdl");
...
void CTripmineGrenadeOLD::WarningThink(void)
{
	// play warning sound
...
void CTripmineGrenadeOLD::PowerupThink(void)
{
	TraceResult tr;
	...
	if (m_hOwner == NULL)
	{
		...
		else
		{
			...
			SetThink(&CTripmineGrenadeOLD::SUB_Remove);
			pev->nextthink = gpGlobals->time + 0.1;
...
void CTripmineGrenadeOLD::KillBeam(void)
{
	if (m_pBeam)
	{
		UTIL_Remove(m_pBeam);
...
void CTripmineGrenadeOLD::MakeBeam(void)
{
	TraceResult tr;
	...
	SetThink(&CTripmineGrenadeOLD::BeamBreakThink);
	pev->nextthink = gpGlobals->time + 0.1;
...
void CTripmineGrenadeOLD::BeamBreakThink(void)
{
	BOOL bBlowup = 0;
...
int CTripmineGrenadeOLD::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
...
void CTripmineGrenadeOLD::Killed(entvars_t *pevAttacker, int iGib)
{
	...
	SetThink(&CTripmineGrenadeOLD::DelayDeathThink);
...
void CTripmineGrenadeOLD::DelayDeathThink(void)
{
	KillBeam();
	TraceResult tr;
...
LINK_ENTITY_TO_CLASS( monster_tripmine_duke, CTripmineGrenade );
...
void CTripmineGrenade :: Spawn( void )
{
	...
	{
		// play deploy sound
		EMIT_SOUND( ENT(pev), CHAN_VOICE, "weapons/tripmine_place.wav", 1.0, ATTN_NORM );
	//	EMIT_SOUND( ENT(pev), CHAN_BODY, "weapons/mine_charge.wav", 0.2, ATTN_NORM ); // chargeup
...
void CTripmineGrenade :: Precache( void )
{
	...
	PRECACHE_SOUND("weapons/mine_activate.wav");
	PRECACHE_SOUND("weapons/mine_charge.wav");
	PRECACHE_SOUND("weapons/tripmine_ignited.wav");
	PRECACHE_SOUND("weapons/tripmine_place.wav");
}
...
void CTripmineGrenade :: PowerupThink( void  )
{
	...
	if (m_hOwner == NULL)
	{
		...
		{
			STOP_SOUND( ENT(pev), CHAN_VOICE, "weapons/tripmine_place.wav" );
			STOP_SOUND( ENT(pev), CHAN_BODY, "weapons/mine_charge.wav" );
	...
	if (gpGlobals->time > m_flPowerUp)
	{
		...
		// play enabled sound
    //    EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "weapons/mine_activate.wav", 0.5, ATTN_NORM, 1.0, 100 );
	}
	pev->nextthink = gpGlobals->time + 0.1;
}
...
void CTripmineGrenade :: MakeBeam( void )
{
	...
	m_pBeam = CBeam::BeamCreate( g_pModelNameLaser, 4 );
	m_pBeam->PointEntInit( vecTmpEnd, entindex() );
...
void CTripmineGrenade :: BeamBreakThink( void  )
{
	...
	{
		bBlowup = 1;
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/tripmine_ignited.wav", 1.0, ATTN_NORM);
...
void CTripmine::Precache( void )
{
	...
	UTIL_PrecacheOther( "monster_tripmine_duke" );
...
void CTripmine::Holster( int skiplocal /* = 0 */ )
{
	...
	if (!m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		...
		pev->nextthink = gpGlobals->time + 0.1;
		m_pPlayer->m_pBeforeAction = NULL;
	}
...
void CTripmine::PrimaryAttack( void )
{
	...
	if (tr.flFraction < 1.0)
	{
		...
		if ( pEntity && !(pEntity->pev->flags & FL_CONVEYOR) )
		{
			...
			CBaseEntity *pEnt = CBaseEntity::Create( "monster_tripmine_duke", tr.vecEndPos + tr.vecPlaneNormal, angles, m_pPlayer->edict() );
