...
TYPEDESCRIPTION	CGargantua::m_SaveData[] = 
{
	...
	DEFINE_FIELD(CGargantua,  m_bHasSeenThePlayer, FIELD_INTEGER),
};
...
int CGargantua::IRelationship(CBaseEntity *pTarget)
{
	...
	if ((pTarget->IsPlayer()) && (m_bHasSeenThePlayer == FALSE))
	{

		CBaseEntity *pPlayer = NULL;
		if ((pPlayer = UTIL_FindEntityInSphere(pPlayer, pev->origin, 700)) != NULL)
		{
			if (FClassnameIs(pPlayer->pev, "player"))
			{
				pTarget->pev->iuser4 = T_ISBIG;
				m_bHasSeenThePlayer = TRUE;
			}
		}
	}
...
class CGargantuaDuke : public CGargantua
{
	public:
		void Spawn();
		void Precache();
...
BOOL CGargantuaDuke::CheckMeleeAttack2(float flDot, float flDist)
{
	//	ALERT(at_aiconsole, "CheckMelee(%f, %f)\n", flDot, flDist);

	if (gpGlobals->time > m_flameTime)
	{
		if (flDot >= 0.8 && flDist > 360.0)
		{
			if (flDist <= GARG_FLAME_LENGTH*4)
				return TRUE;
		}
	}
	return FALSE;
...
void CGargantuaDuke::FireRocket(void)
{
	Vector vecShootOrigin = pev->origin + Vector(0, 0, 115) + gpGlobals->v_forward * 20 + gpGlobals->v_right * 110;


	if (m_iRockets)
		vecShootOrigin = pev->origin + Vector(0, 0, 115) + gpGlobals->v_right * 110;
	else
		vecShootOrigin = pev->origin + Vector(0, 0, 115) + gpGlobals->v_right * -110;
	...
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/rpg_fire.wav", 1.0, 0.5);

	CBaseEntity *pRocket = CBaseEntity::Create("garg_rocket", vecShootOrigin, UTIL_VecToAngles(vecShootDir), edict());
}


int CGargantuaDuke::IRelationship(CBaseEntity *pTarget)
{
	...
	if ((pTarget->IsPlayer()) && (m_bHasSeenThePlayer == FALSE))
	{
		...
		CBaseEntity *pPlayer = NULL;
		if ((pPlayer = UTIL_FindEntityInSphere(pPlayer, pev->origin, 1500)) != NULL)
		{
			if (FClassnameIs(pPlayer->pev, "player"))
			{
				pTarget->pev->iuser4 = T_ISBIG;
...
const char *CGargantuaDuke::pAttackHitSounds[] =
{
	"zombie/claw_strike1.wav",
	"zombie/claw_strike2.wav",
	"zombie/claw_strike3.wav",
...
const char *CGargantuaDuke::pBeamAttackSounds[] =
{
	"garg/gar_flameoff1.wav",
	"garg/gar_flameon1.wav",
	"garg/gar_flamerun1.wav",
...
const char *CGargantuaDuke::pAttackMissSounds[] =
{
	"zombie/claw_miss1.wav",
	"zombie/claw_miss2.wav",
};

const char *CGargantuaDuke::pRicSounds[] =
{
	"debris/metal4.wav",
	"debris/metal6.wav",
	"weapons/ric4.wav",
...
const char *CGargantuaDuke::pFootSounds[] =
{
	"garg/gar_step1.wav",
	"garg/gar_step2.wav",
};
...
const char *CGargantuaDuke::pIdleSounds[] =
{
	"garg/gar_idle1.wav",
	"garg/gar_idle2.wav",
	"garg/gar_idle3.wav",
...
const char *CGargantuaDuke::pAttackSounds[] =
{
	"garg/gar_attack1.wav",
	"garg/gar_attack2.wav",
	"garg/gar_attack3.wav",
...
const char *CGargantuaDuke::pAlertSounds[] =
{
	"garg/gar_alert1.wav",
	"garg/gar_alert2.wav",
	"garg/gar_alert3.wav",
...
const char *CGargantuaDuke::pPainSounds[] =
{
	"garg/gar_pain1.wav",
	"garg/gar_pain2.wav",
	"garg/gar_pain3.wav",
...
const char *CGargantuaDuke::pStompSounds[] =
{
	"garg/gar_stomp1.wav",
};

const char *CGargantuaDuke::pBreatheSounds[] =
{
	"garg/gar_breathe1.wav",
	"garg/gar_breathe2.wav",
	"garg/gar_breathe3.wav",
...
void CGargantuaDuke::Precache()
{

	int i;

	iAgruntMuzzleFlash = PRECACHE_MODEL("sprites/muz4.spr");
	UTIL_PrecacheOther("garg_rocket");

	PRECACHE_MODEL("models/garg_emperor.mdl");
	PRECACHE_MODEL(GARG_EYE_SPRITE_NAME);
	PRECACHE_MODEL(GARG_BEAM_SPRITE_NAME);
	PRECACHE_MODEL(GARG_BEAM_SPRITE2);
	gStompSprite = PRECACHE_MODEL(GARG_STOMP_SPRITE_NAME);
...
void CGargantuaDuke::Spawn()
{
	Precache();

	SET_MODEL(ENT(pev), "models/garg_emperor.mdl");
	UTIL_SetSize(pev, Vector(-64, -64, 0), Vector(64, 64, 128));

	pev->solid = SOLID_SLIDEBOX;
	pev->movetype = MOVETYPE_STEP;
	m_bloodColor = BLOOD_COLOR_YELLOW;
	pev->health = gSkillData.gargantuaHealth*3;
	//pev->view_ofs		= Vector ( 0, 0, 96 );// taken from mdl file
	m_flFieldOfView = 0.5;// width of forward view cone ( as a dotproduct result )
	...
	m_flameTime = gpGlobals->time + 0.5;

	m_bHasSeenThePlayer = FALSE;
}


void CGargantuaDuke::StartTask(Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_FLAME_SWEEP:
		FireRocket();
		m_flWaitFinished = gpGlobals->time + 0.01;
		break;
...
int CGargantuaDuke::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	ALERT(at_aiconsole, "CGargantua::TakeDamage\n");
...
void CGargantuaDuke::RunTask(Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_DIE:
...
LINK_ENTITY_TO_CLASS(monster_gargantua_emperor, CGargantuaDuke);
...
class CGargantuaRocket : public CGrenade
{
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(garg_rocket, CGargantuaRocket);

void CGargantuaRocket::Spawn(void)
{
	Precache();
	// motor
	...
	SetThink(&CGargantuaRocket::IgniteThink);
	SetTouch(&CGargantuaRocket::ExplodeTouch);
...
void CGargantuaRocket::Precache(void)
{
...
void CGargantuaRocket::IgniteThink(void)
{
	...
	// set to accelerate
	SetThink(&CGargantuaRocket::AccelerateThink);
	pev->nextthink = gpGlobals->time + 0.1;
}


void CGargantuaRocket::AccelerateThink(void)
{
