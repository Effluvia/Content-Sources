...
void CHgun::Precache( void )
{
	...
	UTIL_PrecacheOther("blasterprojectile");
}
...
BOOL CHgun::Deploy( )
{
	return DefaultDeploy( "models/v_blaster.mdl", "models/p_hgun.mdl", HGUN_UP, "hive" );
}
...
void CHgun::PrimaryAttack()
{
	...
	vecSrc = vecSrc + gpGlobals->v_up * 4;
	vecSrc = vecSrc + gpGlobals->v_right * 4;

	pHornet = CBaseEntity::Create("blasterprojectile", vecSrc, m_pPlayer->pev->v_angle, m_pPlayer->edict());
	pHornet->pev->velocity = gpGlobals->v_forward * 1200;
	pHornet->pev->angles = UTIL_VecToAngles(pHornet->pev->velocity);

	pHornet->SetThink(&CBlasterProjectile::StartDart);
#endif
