...
void PM_CheckFalling( void )
{
	...
	if (pmove->flFallVelocity > PLAYER_FATAL_FALL_SPEED-10 && pmove->flFallVelocity < PLAYER_FATAL_FALL_SPEED+5)
	{
		pmove->PM_PlaySound(CHAN_BODY, "player/duke/DSCREM04.wav", 1, ATTN_NORM, 0, PITCH_NORM);
	}
	...
	if ( pmove->onground != -1 &&
		...
		else if ( pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED )
		{
			...
			//case 0:
				...
				//break;
			//case 1:
				pmove->PM_PlaySound( CHAN_VOICE, "player/duke/pain39.wav", 1, ATTN_NORM, 0, PITCH_NORM );
			//	break;
			//}
			fvol = 1.0;
		}
		...
		{
			pmove->PM_PlaySound( CHAN_STATIC, "player/duke/LAND.wav", 1, ATTN_NORM, 0, PITCH_NORM );
			fvol = 1.0;	//0.85
		}
