...
void CInputWin32::RunFrame()
{
	...
	if (pContext->_keyFocus)
	{
		...
		{	
			ivgui()->PostMessage((VPANEL)pContext->_keyFocus, new KeyValues("KeyFocusTicked"), NULL, 0);
		}
	}
	...
	if (pContext->_mouseFocus)
	{
		...
		{	
			ivgui()->PostMessage((VPANEL)pContext->_mouseFocus, new KeyValues("MouseFocusTicked"), NULL, 0);
		}
	}
	...
	if (pContext->_keyFocus != wantedKeyFocus)
	{
		if (pContext->_keyFocus != NULL)
		{
			...
			ivgui()->PostMessage((VPANEL)pContext->_keyFocus, new KeyValues("KillFocus"), NULL, 0);
		...
		if (wantedKeyFocus != NULL)
		{
			...
			ivgui()->PostMessage((VPANEL)wantedKeyFocus, new KeyValues("SetFocus"), NULL, 0);
...
void CInputWin32::SetMouseFocus(VPANEL newMouseFocus)
{
	...
	if ((VPANEL)pContext->_mouseOver != newMouseFocus || (!pContext->_mouseCapture && (VPANEL)pContext->_mouseFocus != newMouseFocus) )
	{
		...
		if (pContext->_oldMouseFocus != NULL)
		{
			...
			{
				ivgui()->PostMessage((VPANEL)pContext->_oldMouseFocus, new KeyValues("CursorExited"), NULL, 0);
			}
		}
		...
		if (pContext->_mouseOver != NULL)
		{
			...
			{
				ivgui()->PostMessage((VPANEL)pContext->_mouseOver, new KeyValues("CursorEntered"), NULL, 0);
			}
		}
...
void CInputWin32::UpdateMouseFocus(int x, int y)
{
	...
	if (oldFocus != focus)
	{
		...
		if (focus)
		{
			ivgui()->DPrintf2("mouse over: (%s, %s)\n", focus->GetName(), focus->LOCAL_GetClassName());
		}
		else
		{
...
void CInputWin32::SetMouseCapture(VPANEL panel)
{
	...
	{
		ivgui()->PostMessage((VPANEL)pContext->_mouseCapture, new KeyValues("MouseCaptureLost"), NULL, 0);
	}
...
{
	//MODDD - how can this be???  (common crash on closing)
	// doesn't really help actually.  ehhhh.
	// But the crash is avoided by making the version 027 instead of 026.
	// Uses the surface supplied by this codebase instead of from
	// hl.exe, maybe?  weeeeeeeird
	ISurface* surfTest = surface();
	if(surfTest == 0 || (void*)surfTest == (void*)0xFFFFFFFF){
		return;
	}


	if ( surfTest->HasCursorPosFunctions() ) // does the surface export cursor functions for us to use?
	{
		surfTest->SurfaceGetCursorPos(x,y);
	}
	else
	{
		...
		// translate into coordinates relative to surface
		int px, py, pw, pt;
		surfTest->GetAbsoluteWindowBounds(px, py, pw, pt);
		x -= px;
		y -= py;
	}
}
...
void CInputWin32::InternalCursorMoved(int x, int y)
{
	...
	if (pContext->_mouseCapture)
	{
		...
		{
			ivgui()->PostMessage((VPANEL)pContext->_mouseOver, new KeyValues("CursorMoved", "xpos", x, "ypos", y), NULL, 0);
		}

		// the panel with mouse capture gets all messages
		ivgui()->PostMessage((VPANEL)pContext->_mouseCapture, new KeyValues("CursorMoved", "xpos", x, "ypos", y), NULL, 0);
	}
	else if (pContext->_mouseFocus != NULL)
	{
		...
		ivgui()->PostMessage((VPANEL)pContext->_mouseFocus, new KeyValues("CursorMoved", "xpos", x, "ypos", y), NULL, 0);
	}
 }
...
void CInputWin32::InternalMousePressed(MouseCode code)
{
	...
	if ( pContext->_mouseCapture && IsChildOfModalPanel((VPANEL)pContext->_mouseCapture))
	{
		...
		if ( pContext->_mouseOver && pContext->_mouseOver != pContext->_mouseCapture && pContext->_mouseOver->HasParent(pContext->_mouseCapture) )
		{
			ivgui()->PostMessage((VPANEL)pContext->_mouseOver, new KeyValues("MousePressed", "code", code), NULL, 0);
		}
		else
		{
		...
		// the panel with mouse capture gets all messages
		ivgui()->PostMessage((VPANEL)pContext->_mouseCapture, new KeyValues("MousePressed", "code", code), NULL, 0);
	...
	else if ( (pContext->_mouseFocus != NULL) && IsChildOfModalPanel((VPANEL)pContext->_mouseFocus) )
	{
		...
		ivgui()->PostMessage((VPANEL)pContext->_mouseFocus, new KeyValues("MousePressed", "code", code), NULL, 0);
//		ivgui()->DPrintf2("MousePressed: (%s, %s)\n", _mouseFocus->GetName(), _mouseFocus->LOCAL_GetClassName());
	}
...
void CInputWin32::InternalMouseDoublePressed(MouseCode code)
{
	...
	if ( pContext->_mouseCapture && IsChildOfModalPanel((VPANEL)pContext->_mouseCapture))
	{
		...
		if ( pContext->_mouseOver && pContext->_mouseOver != pContext->_mouseCapture && pContext->_mouseOver->HasParent(pContext->_mouseCapture) )
		{
			ivgui()->PostMessage((VPANEL)pContext->_mouseOver, new KeyValues("MouseDoublePressed", "code", code), NULL, 0);
		}

		// the panel with mouse capture gets all messages
		ivgui()->PostMessage((VPANEL)pContext->_mouseCapture, new KeyValues("MouseDoublePressed", "code", code), NULL, 0);
	}
	else if ( (pContext->_mouseFocus != NULL) && IsChildOfModalPanel((VPANEL)pContext->_mouseFocus))
	{			
		//tell the panel with the mouseFocus that the mouse was double presssed
		ivgui()->PostMessage((VPANEL)pContext->_mouseFocus, new KeyValues("MouseDoublePressed", "code", code), NULL, 0);
	}
...
void CInputWin32::InternalMouseReleased(MouseCode code)
{
	...
	if (pContext->_mouseCapture && IsChildOfModalPanel((VPANEL)pContext->_mouseCapture))
	{
		...
		if (pContext->_mouseOver && pContext->_mouseOver != pContext->_mouseCapture && pContext->_mouseOver->HasParent(pContext->_mouseCapture))
		{
			ivgui()->PostMessage((VPANEL)pContext->_mouseOver, new KeyValues("MouseReleased", "code", code), NULL, 0);
		}

		// the panel with mouse capture gets all messages
		ivgui()->PostMessage((VPANEL)pContext->_mouseCapture, new KeyValues("MouseReleased", "code", code), NULL, 0);
	}
	else if ((pContext->_mouseFocus != NULL) && IsChildOfModalPanel((VPANEL)pContext->_mouseFocus))
	{
		//tell the panel with the mouseFocus that the mouse was release
		ivgui()->PostMessage((VPANEL)pContext->_mouseFocus, new KeyValues("MouseReleased", "code", code), NULL, 0);
	}
}
...
void CInputWin32::InternalMouseWheeled(int delta)
{
	...
	if ((pContext->_mouseFocus != NULL) && IsChildOfModalPanel((VPANEL)pContext->_mouseFocus))
	{
		...
		ivgui()->PostMessage((VPANEL)pContext->_mouseFocus, new KeyValues("MouseWheeled", "delta", delta), NULL, 0);
	}
}
...
void CInputWin32::PostKeyMessage(KeyValues *message)
{
	...
	if( (pContext->_keyFocus!= NULL) && IsChildOfModalPanel((VPANEL)pContext->_keyFocus))
	{
		...
		ivgui()->PostMessage((VPANEL)pContext->_keyFocus, message, NULL, 0);
	}
	else
	{
		message->deleteThis();
