...
void PropertySheet::ResetAllData()
{
	...
	{
		ivgui()->PostMessage(_pages[i]->GetVPanel(), new KeyValues("ResetData"), GetVPanel(), 0);
	}
}
...
void PropertySheet::ApplyChanges()
{
	...
	for (int i = 0; i < _pages.GetCount(); i++)
	{
		ivgui()->PostMessage(_pages[i]->GetVPanel(), new KeyValues("ApplyChanges"), GetVPanel(), 0);
	}
}
...
void PropertySheet::ChangeActiveTab(int index)
{
	...
	if (_activePage)
	{
		ivgui()->PostMessage(_activePage->GetVPanel(), new KeyValues("PageHide"), GetVPanel(), 0);
		KeyValues *msg = new KeyValues("PageTabActivated");
		msg->SetPtr("panel", (Panel *)NULL);
		ivgui()->PostMessage(_activePage->GetVPanel(), msg, GetVPanel(), 0);
	...
	// notify
	ivgui()->PostMessage(_activePage->GetVPanel(), new KeyValues("PageShow"), GetVPanel(), 0);
