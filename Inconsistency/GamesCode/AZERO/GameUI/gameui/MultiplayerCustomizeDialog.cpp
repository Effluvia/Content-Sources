...
: Frame(NULL, "MultiplayerCustomizeDialog"), m_pMenuTarget( menuTarget )
{
	...
	//MODDD - undefined method.  How about this instead?
	//SetParent( GetGameUIRootPanel() );
	SetParent(GetGameUIBasePanel());
	...
	//MODDD - TODO.  Not sure how to convey this yet.
...
void CMultiplayerCustomizeDialog::InitModelList( CLabeledCommandComboBox *cb )
{
	...
	while ( fn )
	{
		...
		if ( fn[ 0 ] && fn[0] != '.' )
		{
			...
			//MODDD - Q_strstr now returns 'const char*' since strstr does.
			// Adjustments made.
			const char *spot = Q_strstr( modelname, ".mdl" );
			if ( spot )
			{
				//MODDD - do another way
				//*spot = 0;
				int relativeTo = spot - modelname;
				modelname[relativeTo] = 0;
			}
