...
public:
	...
	void OnDisconnectFromServer(int eSteamLoginFailure, char* username);

	bool FindPlatformDirectory(char *platformDir, int bufferSize);



	void RunVguiTestScript(char *script)
	{
		/*
		Panel *pPVar1;
		TestScriptController *this_00;

		pPVar1 = Get((PHandle *)&this->m_hTestScriptController);
		if (pPVar1 != (Panel *)0x0) {
			this_00 = (TestScriptController *)Get((PHandle *)&this->m_hTestScriptController);
			ExecuteScript(this_00,script);
			return;
		}
...

	{
		...
		(**(code **)(*(int *)baseuifuncs + 0x20))(baseuifuncs);
		*/
		//SetBackgroundRenderState(staticPanel,BACKGROUND_DESKTOPIMAGE);
		return;
	}
...
private:
	//MODDD - why even have these prototypes if their methods were dummied out as-is?
	/*
