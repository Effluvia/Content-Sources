...
// Viewmodel bodygroups
enum
{
	HGREN_BODYGROUP_GRENADE = 0,
	HGREN_BODYGROUP_BODY
};
enum
{
	HGREN_GRENADE_SHOW = 0,
	HGREN_GRENADE_HIDE
};
...
bool CWeaponHandGrenade::Deploy( void )
{
	...
	if ( BaseClass::Deploy() )
	{
		SetViewModelBodygroup( HGREN_BODYGROUP_GRENADE, HGREN_GRENADE_SHOW );
		return true;
	}
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponHandGrenade::Reload( void )
{
	if ( !HasPrimaryAmmo() )
		return false;
...
#ifdef HOE_THIRDPERSON
	ToHL2Player(GetOwner())->DoAnimationEvent( PLAYERANIMEVENT_GRENADE1_DRAW );
#endif
...
// Input  : *pOwner - 
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::DecrementAmmo( CBaseCombatCharacter *pOwner )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::ItemPostFrame( void )
{
	if( m_fDrawbackFinished )
	{
		...
		if (pOwner)
		{
...
#ifdef HOE_THIRDPERSON
					ToHL2Player(GetOwner())->DoAnimationEvent( PLAYERANIMEVENT_GRENADE1_THROW );
#endif
			...
			case GRENADE_PAUSED_SECONDARY:
				...
				{
					// Hide the grenade
					SetViewModelBodygroup( HGREN_BODYGROUP_GRENADE, HGREN_GRENADE_HIDE );

					SendWeaponAnim( ACT_VM_SECONDARYATTACK );
#ifdef HOE_THIRDPERSON
					ToHL2Player(GetOwner())->DoAnimationEvent( PLAYERANIMEVENT_GRENADE2_THROW );
#endif
	...
	// check a throw from vecSrc.  If not valid, move the position back along the line to vecEye
void CWeaponHandGrenade::CheckThrowPosition( CBasePlayer *pPlayer, const Vector &vecEye, Vector &vecSrc )
{
	trace_t tr;
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::ThrowGrenade( CBasePlayer *pPlayer )
{
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::LobGrenade( CBasePlayer *pPlayer )
{
	Vector	vecEye = pPlayer->EyePosition();
	Vector	vForward, vRight;
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::RollGrenade( CBasePlayer *pPlayer )
{
...
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::SetViewModel( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponHandGrenade::SetViewModelBodygroup( int iGroup, int iValue )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( !pOwner )
		return;
