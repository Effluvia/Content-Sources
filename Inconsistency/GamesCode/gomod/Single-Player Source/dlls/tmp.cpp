...
void CTMP::Spawn( )
{
	...
	pev->classname = MAKE_STRING("weapon_tmp"); // hack to allow for old names
	Precache( );
...
int CTMP::AddToPlayer( CBasePlayer *pPlayer )
{
...
void CTMP::PrimaryAttack()
{
  ...
  int flags;
	...
	PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_tmp, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, vecDir.x, vecDir.y, 0, 0, 0, 0 );
...
DefaultReload( 35, TMP_RELOAD, 1.0 );
...
/***************************************
Nuevas Physics Para la TMP
Reaccionan a explosiones etc
***************************************/
float CTMP :: DamageForce( float damage )
{ 
...
int CTMP :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	float	flTake;
	Vector	vecDir;
...
class CTMPAmmoClip : public CBasePlayerAmmo
{
	void Spawn( void )
	{ 
		Precache( );
	...
	{ 
		int bResult = (pOther->GiveAmmo( 35, "ammo_TMP", 120) != -1);
		if (bResult)
		{
...
LINK_ENTITY_TO_CLASS( ammo_TMP, CTMPAmmoClip );




class CTMPChainammo : public CBasePlayerAmmo
{
	void Spawn( void )
	{ 
		Precache( );
