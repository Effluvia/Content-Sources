...
public:
	...
	// New for Portals
	void EXPORT PortalTouch( CBaseEntity *pOther );
...
//	pevToucher->velocity = pevToucher->basevelocity = g_vecZero;
	pevToucher->velocity = pevToucher->velocity;
}
...
// New Code For Portals
// -- Portal Start 


void CBaseTrigger :: PortalTouch( CBaseEntity *pOther )
{
	entvars_t* pevToucher = pOther->pev;
	edict_t *pentTarget = NULL;
	...
	pentTarget = FIND_ENTITY_BY_CLASSNAME( pentTarget, "info_portal_destination" );
	//if (FNullEnt(pentTarget))
		// return;	
...
//	pevToucher->fixangle = TRUE;
//	pevToucher->velocity = pevToucher->basevelocity = g_vecZero;
	pevToucher->velocity = pevToucher->velocity + (pevToucher->velocity / 2);
}
...
void CTriggerPortal :: Spawn( void )
{
	...
	SetTouch( PortalTouch );
}


LINK_ENTITY_TO_CLASS( info_portal_destination, CPointEntity );
