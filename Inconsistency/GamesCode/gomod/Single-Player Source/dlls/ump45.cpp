...
void CUMP45::Spawn( )
{
	...
	pev->classname = MAKE_STRING("weapon_ump45"); // hack to allow for old names
	Precache( );
...
void CUMP45::Precache( void )
{
	...
	PRECACHE_SOUND ("weapons/ump45-1.wav");// H to the K
	...
	m_FireUMP = PRECACHE_EVENT( 1, "events/ump45.sc" );
...
BOOL CUMP45::Deploy( )
{
	return DefaultDeploy( "models/v_ump45.mdl", "models/p_ump45.mdl", UMP45_DRAW, "ump45" );
}
...
void CUMP45::PrimaryAttack()
{
...
#endif
	{
		...
		vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, VECTOR_CONE_6DEGREES, 8192, BULLET_PLAYER_UMP, 2, 0, m_pPlayer->pev, m_pPlayer->random_seed );
	}
	else
	{
	
			// single player spread
	vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, VECTOR_CONE_3DEGREES, 8192, BULLET_PLAYER_UMP, 2, 0, m_pPlayer->pev, m_pPlayer->random_seed );
  ...
  int flags;
	...
	PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_FireUMP, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, vecDir.x, vecDir.y, 0, 0, 0, 0 );
...
DefaultReload( 30,	UMP45_RELOAD, 2 );
...
/***************************************
Nuevas Physics Para la UMP45
Reaccionan a explosiones etc
***************************************/
float CUMP45 :: DamageForce( float damage )
{ 
...
int CUMP45 :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	float	flTake;
	Vector	vecDir;
...
class CUMP45AmmoClip : public CBasePlayerAmmo
{
	void Spawn( void )
	{ 
		Precache( );
	...
	{ 
		int bResult = (pOther->GiveAmmo( HKMP5_MAX_CARRY, "ammo_UMP45", HKMP5_MAX_CARRY) != -1);
		if (bResult)
		{
...
LINK_ENTITY_TO_CLASS( ammo_UMP45, CUMP45AmmoClip );



/*
class CUMP45Chainammo : public CBasePlayerAmmo
{
	void Spawn( void )
	{ 
		Precache( );
...
LINK_ENTITY_TO_CLASS( ammo_ump45, CUMP45Chainammo );
