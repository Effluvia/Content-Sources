...
class CSentryAlly : public CBaseTurret
{
public:
	void Spawn( );
	void Precache(void);
...
LINK_ENTITY_TO_CLASS( monster_sentry_ally, CSentryAlly );

void CSentryAlly::Precache()
{
	CBaseTurret::Precache( );
	PRECACHE_MODEL ("models/gomod/sentry.mdl");	
}

void CSentryAlly::Spawn()
{ 
	Precache( );
	SET_MODEL(ENT(pev), "models/gomod/sentry.mdl");
	pev->health			= gSkillData.sentryHealth;
  m_HackedGunPos		= Vector( 0, 0, 0 );
	pev->view_ofs.z		= 48;
	m_flMaxWait = 1E6;
...
void CSentryAlly::Shoot(Vector &vecSrc, Vector &vecDirToEnemy)
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, TURRET_RANGE, BULLET_MONSTER_MP5, 1 );
...
int CSentryAlly::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	if ( !pev->takedamage )
		return 0;
...
void CSentryAlly::SentryTouch( CBaseEntity *pOther )
{
	if ( pOther && (pOther->IsPlayer() || (pOther->pev->flags & FL_MONSTER)) )
	{
		TakeDamage(pOther->pev, pOther->pev, 0, 0 );
...
void CSentryAlly ::	SentryDeath( void )
{
	BOOL iActive = FALSE;
