...
void EV_RemoveTool( struct event_args_s *args  );//REMOVE TOOL
void EV_Punch( struct event_args_s *args  );//PUNCHES
...
//void EV_Duplicator( struct event_args_s *args  );//DUPLICATOR TOOL
//void EV_Duplicator2( struct event_args_s *args  );//DUPLICATOR2 TOOL
void EV_FireM4A1( struct event_args_s *args ); //CS M4A1 WEAPON
void EV_FireM4A1_UNSIL( struct event_args_s *args ); //CS M4A1_UNSIL WEAPON
...
void EV_FireUMP( struct event_args_s *args ); //CS UMP WEAPON
...
void EV_HLDM_FireBulletsPoserTool( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float flDistance, int iBulletType, int iTracerFreq, int *tracerCount, float flSpreadX, float flSpreadY )
{
	int i;
	pmtrace_t tr;
	int iShot;
...
void EV_HLDM_FireBulletsDuplicator( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float flDistance, int iBulletType, int iTracerFreq, int *tracerCount, float flSpreadX, float flSpreadY )
{
	int i;
	pmtrace_t tr;
	int iShot;
...
void EV_HLDM_FireBulletsDuplicatorSelect( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float flDistance, int iBulletType, int iTracerFreq, int *tracerCount, float flSpreadX, float flSpreadY )
{
	int i;
	pmtrace_t tr;
	int iShot;
...
void EV_m3Fire( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( M3_SHOOT1, 2 );
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m3-1.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBulletsRemoveTool( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBulletsRemoveTool( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FirePoser( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( POSERTOOL_SHOOT + gEngfuncs.pfnRandomLong(0,0), 0 );
...
void EV_Duplicator( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( DUPLICATOR_SHOOT + gEngfuncs.pfnRandomLong(0,0), 0 );
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBulletsDuplicator( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBulletsDuplicator( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
/*
void EV_Duplicator2( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBulletsDuplicatorSelect( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBulletsDuplicatorSelect( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBulletsPhysGun( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_NONE, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBulletsPhysGun( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_NONE, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireP228( event_args_t *args )
{
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/p228_fire1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/p228_fire2.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM );
		break;
	}
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_P228, 0, 0, args->fparam1, args->fparam2 );
}
...
void EV_FireDEAGLE( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation( DEAGLE_SHOOT1 + gEngfuncs.pfnRandomLong(0,1), 1 );
...
void EV_FireFIVESEVEN( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation( FIVESEVEN_SHOOT1 + gEngfuncs.pfnRandomLong(0,1), 1 );
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/fiveseven-1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/fiveseven-1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM );
		break;
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_FIVE, 0, 0, args->fparam1, args->fparam2 );
}
...
void EV_FireGLOCK18( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation( GLOCK18_SHOOT3 + gEngfuncs.pfnRandomLong(0,0), 0 );
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_GLOCK18, 0, 0, args->fparam1, args->fparam2 );
}
...
//Only predict the miss sounds, hit sounds are still played 
//server side, so players don't get the wrong idea.
void EV_Punch( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation( PUNCH_PUNCH1, 1 );

		switch( (g_iPunch++) % 3 )
		{
			case 0:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( PUNCH_PUNCH1, 1 ); break;
				
			case 1:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( PUNCH_PUNCH2, 1 ); break;
			case 2:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( PUNCH_PUNCH1, 1 ); break;
		}
	}
...
void EV_FireHKMP5( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( HKMP5_SHOOT1 + gEngfuncs.pfnRandomLong(0,2), 2 );
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_HKMP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_HKMP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireUMP( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ump45-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ump45-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
...
void EV_FireAK47( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_AK47, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_AK47, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireM4A1( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_M4A1, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_M4A1, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireM4A1_UNSIL( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( M4A1_SHOOT1_UNSIL + gEngfuncs.pfnRandomLong(0,2), 2 );
...
void EV_FireUSP( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_USP, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_USP, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
void EV_FireP90( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/p90-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/p90-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
...
void EV_FireMAC10( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MAC10, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MAC10, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
