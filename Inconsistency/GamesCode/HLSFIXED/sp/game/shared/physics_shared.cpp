...
void PrecachePhysicsSounds()
{
	...
	for ( int i = 0; i < physprops->SurfacePropCount(); i++ )
	{
		...
		//HL:S
		//Precache HL1 Movement Sounds
		if (pprop->sounds.stepleft != 0)
		{
			//Step Left
			char hl1left[50] = "HL1";
			const char* stepLeft = physprops->GetString(pprop->sounds.stepleft);
			strcat_s(hl1left, stepLeft);
			CBaseEntity::PrecacheScriptSound(hl1left);

			//Step Right
			char hl1right[50] = "HL1";
			const char* stepRight = physprops->GetString(pprop->sounds.stepright);
			strcat_s(hl1right, stepRight);
			CBaseEntity::PrecacheScriptSound(hl1right);
		}
