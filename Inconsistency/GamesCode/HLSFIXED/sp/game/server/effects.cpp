...
CGib *CEnvShooter::CreateGib ( void )
{
	...
	if (m_clrRender->r == 0 && m_clrRender->g == 0 && m_clrRender->b == 0)
	{
		m_clrRender.SetR(255);
		m_clrRender.SetG(255);
		m_clrRender.SetB(255);
	}
...
void CItemSoda::CanThink ( void )
{
	...
	//HL:S
	//Changing the soda's bounding box allows the player to pick it up.
