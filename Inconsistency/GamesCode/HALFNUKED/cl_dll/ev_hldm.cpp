...
void EV_FireDeva(struct event_args_s *args);
void EV_FireDeva2(struct event_args_s *args);
...
void EV_FireGlock1( event_args_t *args )
{
	...
	//	gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? GLOCK_SHOOT_EMPTY : GLOCK_SHOOT, 2 );
		...
		V_PunchAxis( 0, -0.75 );	//duke pistol aim punch
	}

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, -right, up, 20, -5, -4 );

	EV_EjectBrass(ShellOrigin, ShellVelocity, angles[YAW], shell, TE_BOUNCE_SHELL);

	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/pistol_fire.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
...
void EV_FireMP5( event_args_t *args )
{
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/chaingun_fire.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
void EV_FireMP52( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation( MP5_FIRE1, 2 );
		V_PunchAxis( 0, -10 );
	}
...
void EV_FireRpg( event_args_t *args )
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/rpg_fire.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
//	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_ITEM, "weapons/glauncher.wav", 0.7, ATTN_NORM, 0, PITCH_NORM );
	...
	if ( EV_IsLocal( idx ) )
	{
		...
		V_PunchAxis(0, -2.0);
		V_PunchAxis(2, gEngfuncs.pfnRandomFloat(-0.25,0.25));
	}
}

void EV_FireDeva(event_args_t *args)
{
	int idx;
	vec3_t origin;
	...
	if (EV_IsLocal(idx))
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation(DEVASTATOR_FIRELEFT, 1);

		V_PunchAxis(0, -2.0);
		V_PunchAxis(2, gEngfuncs.pfnRandomFloat(-0.25, 0.25));
	}
}
void EV_FireDeva2(event_args_t *args)
{
	int idx;
	vec3_t origin;
	...
	if (EV_IsLocal(idx))
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation(DEVASTATOR_FIRERIGHT, 3);
...
void EV_HornetGunFire( event_args_t *args )
{
	...
	*/
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/blaster_fire.wav", 1, ATTN_NORM, 0, gEngfuncs.pfnRandomLong(97, 104));
}
//======================
//	   HORNET END
//======================
...
void EV_TripmineFire( event_args_t *args )
{
	...
	//Hit something solid
	if ( tr.fraction < 1.0 )
		 gEngfuncs.pEventAPI->EV_WeaponAnimation ( TRIPMINE_PLACE, 0 );
