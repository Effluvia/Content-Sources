...
void CBasePlayer :: DeathSound( void )
{
	...
	*/
	...
	EMIT_SOUND(ENT(pev), CHAN_STATIC, "player/duke/DMDEATH.wav", 1, ATTN_NORM);
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	pevAttacker->iuser4 = T_KILLEDENEMY;
...
{
	ammo_chaingun = AmmoInventory(GetAmmoIndex("chaingun"));
	...
	ammo_rockets = AmmoInventory( GetAmmoIndex( "devastator_rockets" ) );
...
//			ALERT( at_console, "%s : %f\n", STRING( pObject->pev->classname ), flDot );
	...
	else
	{
		if (m_afButtonPressed & IN_USE)
		{
			if (m_flTimeUse < gpGlobals->time)
			{
				if (RANDOM_LONG(0, 10))
				{
					EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/duke/taunts/USEDENY.wav", 1.0, ATTN_NORM);
					m_flTimeUse = gpGlobals->time + 0.25;
				}
				else
				{
					EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/duke/taunts/WHRSIT05.wav", 1.0, ATTN_NORM);
					m_flTimeUse = gpGlobals->time + 1.25;
				}
			}
		}
	}
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			{//splat
				...
				EMIT_SOUND(ENT(pev), CHAN_VOICE, "player/duke/DMDEATH.wav", 1, ATTN_NORM);
			}
...
// dont let uninitialized value here hurt the player
	...
	SET_MODEL(ENT(pev), "models/duke.mdl");
	...
	GiveNamedItem("item_duke");

	m_pBeforeAction = NULL;
...
#endif
	{
		GiveNamedItem("weapon_kick");
		GiveNamedItem("weapon_intro");
	}
}


void CBasePlayer::PrecacheTaunt(void)
{
	m_flTimeTaunt = 0.0;
	m_flTimePain = 0.0;
}
void CBasePlayer::Taunt()
{
	...
	if (pev->iuser4 == T_INTRO)
	{
		ALERT(at_aiconsole, "[TAUNT] :: DUKE ARRIVED, PLAYING HIS INTRO\n");

		switch (RANDOM_LONG(0, 10))
		{
		case 1:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/ABORT01.wav", T_VOLUME, T_ATTN); break;
		case 2:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/behere.wav", T_VOLUME, T_ATTN); break;
		case 3:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/behere2.wav", T_VOLUME, T_ATTN); break;
		case 4:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/bornready.wav", T_VOLUME, T_ATTN); break;
		case 5:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/CHEW05.wav", T_VOLUME, T_ATTN); break;
		case 6:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/CON03.wav", T_VOLUME, T_ATTN); break;
		case 7:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/dejavu.wav", T_VOLUME, T_ATTN); break;
		case 8:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/dreams.wav", T_VOLUME, T_ATTN); break;
		case 9:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/old.wav", T_VOLUME, T_ATTN); break;
		case 10:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/seatbelts.wav", T_VOLUME, T_ATTN); break;
		default:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/intro/VACATN01.wav", T_VOLUME, T_ATTN); break;
		}
		m_flTimeTaunt = gpGlobals->time + T_WAITTIME;
		return;
	}

	if (pev->iuser4 == T_KILLEDBOSS)
	{
		m_flTimeTaunt = gpGlobals->time + T_WAITTIME;
		ALERT(at_aiconsole, "[TAUNT] :: KILLED THE BOSS!\n");
		EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/special/wontbeinthesequel.wav", T_VOLUME, T_ATTN);
		return;
	}

	if (pev->iuser4 == T_ISBIG)
	{
		ALERT(at_aiconsole, "[TAUNT] :: DAMN IT's BIG!\n");

		switch (RANDOM_LONG(0, 1))
		{
		case 1:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/special/WHIPYU01.wav", T_VOLUME, T_ATTN); break;
		default:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/special/VO_E5L7_Duke_FlamingAlienBitch.wav", T_VOLUME, T_ATTN); break;
		}
	...
	if (pev->iuser4 == T_KILLEDBIG)
	{
		m_flTimeTaunt = gpGlobals->time + T_WAITTIME;
		ALERT(at_aiconsole, "[TAUNT] :: KILLED A BIG PIECE OF GARBAGE!\n");
		EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/special/DIESOB03.wav", T_VOLUME, T_ATTN);
		return;
	}

	if (pev->iuser4 == T_TOOKDAMAGE && (m_flTimePain < gpGlobals->time))
	{
		ALERT(at_aiconsole, "[TAUNT] :: SHIT I TOOK DAMAGE!\n");


		if (pev->health >= 35)
		{
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pain/GASPS07.wav", T_VOLUME, T_ATTN);
			m_flTimePain = gpGlobals->time + T_DAMAGEWAITTIME;
			return;
		}
			...
			if (RANDOM_LONG(0, 30))
				...
				{
				case 1:
					EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pain/DSCREM15.wav", T_VOLUME, T_ATTN); break;
				case 2:
					EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pain/DSCREM16.wav", T_VOLUME, T_ATTN); break;
				default:
					EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pain/DSCREM17.wav", T_VOLUME, T_ATTN); break;
				}
			else
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pain/DSCREM38.wav", T_VOLUME, T_ATTN);

		m_flTimePain = gpGlobals->time + T_DAMAGEWAITTIME * 1.5;
	...
	if (m_flTimeTaunt >= gpGlobals->time)
	{
		ALERT(at_aiconsole, "[TAUNT] :: PLAYER ATTEMPTED TO TAUNT, IT WAS TOO SOON [ %f / %f ]", m_flTimeTaunt, gpGlobals->time);
		return;
	}

	int tauntChance = RANDOM_LONG(0, 30);

	if (pev->iuser4 == T_PICKEDUPHEALTH && pev->health <= 35)
	{
		ALERT(at_aiconsole, "[TAUNT] :: PICKED UP HEALTH\n");

		switch (RANDOM_LONG(0, 2))
		{
		case 1:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/NEEDED03.wav", T_VOLUME, T_ATTN); break;
		case 2:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/Convenient.wav", T_VOLUME, T_ATTN); break;
		default:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/MuchBetter.wav", T_VOLUME, T_ATTN); break;
		}
	...
	if (pev->iuser4 == T_PICKEDUPWEAPON)
	{
		ALERT(at_aiconsole, "[TAUNT] :: PICKED UP WEAPON\n");

		switch (RANDOM_LONG(0, 9))
		{
		case 1:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/HAIL01.wav", T_VOLUME, T_ATTN); break;
		case 2:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/PAYBACKTIME.wav", T_VOLUME, T_ATTN); break;
		case 3:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/ComeGetSome.wav", T_VOLUME, T_ATTN); break;
		case 4:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/FreshHardware.wav", T_VOLUME, T_ATTN); break;
		case 5:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/Groovy.wav", T_VOLUME, T_ATTN); break;
		case 6:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/JustLikeChristmas.wav", T_VOLUME, T_ATTN); break;
		case 7:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/MoreEquipement.wav", T_VOLUME, T_ATTN); break;
		case 8:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/COOL01.wav", T_VOLUME, T_ATTN); break;
		case 9:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/WANSOM4A.wav", T_VOLUME, T_ATTN); break;
		default:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/pickup/GROOVY02.wav", T_VOLUME, T_ATTN); break;
		}
	...
	if (pev->iuser4 == T_KILLEDFRIEND)
	{
		ALERT(at_aiconsole, "[TAUNT] :: KILLED A FRIEND\n");

		switch (RANDOM_LONG(0, 2))
		{
		case 1:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedfriend/DAMNIT04.wav", T_VOLUME, T_ATTN); break;
		case 2:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedfriend/DAMN03.wav", T_VOLUME, T_ATTN); break;
		default:
			EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedfriend/HAPPEN01.wav", T_VOLUME, T_ATTN); break;
		}
	...
	if (tauntChance >= T_TAUNTCHANCE)
	{
		m_flTimeTaunt = gpGlobals->time + T_WAITTIME;

		if (pev->iuser4 == T_PICKEDUPAMMO)
		{
			ALERT(at_aiconsole, "[TAUNT] :: PICKED UP AMMO\n");
		...
		if (pev->iuser4 == T_KILLEDGIBBED)
		{
			ALERT(at_aiconsole, "[TAUNT] :: GIB TAUNTED\n");
			switch (RANDOM_LONG(0, 13))
			{
			case 1:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/LOTOFGUTS.wav", T_VOLUME, T_ATTN); break;
			case 2:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/whatamess.wav", T_VOLUME, T_ATTN); break;
			case 3:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/QUITBLEEDING.wav", T_VOLUME, T_ATTN); break;
			case 4:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/WHOSNEXT.wav", T_VOLUME, T_ATTN); break;
			case 5:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/BRAINANDBLOOD.wav", T_VOLUME, T_ATTN); break;
			case 6:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/CHETLANDS.wav", T_VOLUME, T_ATTN); break;
			case 7:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/gutsglory.wav", T_VOLUME, T_ATTN); break;
			case 8:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/bigbleed.wav", T_VOLUME, T_ATTN); break;
			case 9:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/SUKIT01.wav", T_VOLUME, T_ATTN); break;
			case 10:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/HOLYCW01.wav", T_VOLUME, T_ATTN); break;
			case 11:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/HOLYSH02.wav", T_VOLUME, T_ATTN); break;
			case 12:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/fallenapart.wav", T_VOLUME, T_ATTN); break;
			case 13:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/restinpieces.wav", T_VOLUME, T_ATTN); break;
			default:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/ANOTHERDAY.wav", T_VOLUME, T_ATTN); break;
			}
			return;
		}

		if (pev->iuser4 == T_KILLEDENEMY)
		{
			ALERT(at_aiconsole, "[TAUNT] :: KILL TAUNTED\n");
			switch (RANDOM_LONG(0, 25))
			{
			case 1:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/EATSHT01.wav", T_VOLUME, T_ATTN); break;
			case 2:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/FORCE01.wav", T_VOLUME, T_ATTN); break;
			case 3:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/GOTHRT01.wav", T_VOLUME, T_ATTN); break;
			case 4:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/YOUSUCK.wav", T_VOLUME, T_ATTN); break;
			case 5:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/YIPPIE01.wav", T_VOLUME, T_ATTN); break;
			case 6:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/HURTSTOBEYOU.wav", T_VOLUME, T_ATTN); break;
			case 7:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/IMGOOD12.wav", T_VOLUME, T_ATTN); break;
			case 8:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/INHELL01.wav", T_VOLUME, T_ATTN); break;
			case 9:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/PIECE02.wav", T_VOLUME, T_ATTN); break;
			case 10:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/RIPEM08.wav", T_VOLUME, T_ATTN); break;
			case 11:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/ROCKIN02.wav", T_VOLUME, T_ATTN); break;
			case 12:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/headsup.wav", T_VOLUME, T_ATTN); break;
			case 13:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/VO_E5L5_Duke_ICU.wav", T_VOLUME, T_ATTN); break;
//			case 14:
//				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/VO_MQ_Duke_BigBang.wav", T_VOLUME, T_ATTN); break;
			case 14:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/VO_MQ_Duke_Cobra.wav", T_VOLUME, T_ATTN); break;
			case 15:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/icameisaw.wav", T_VOLUME, T_ATTN); break;
			case 16:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/asskickingtime.wav", T_VOLUME, T_ATTN); break;
			case 17:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/allyougot.wav", T_VOLUME, T_ATTN); break;
			case 18:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/bringiton.wav", T_VOLUME, T_ATTN); break;
			case 19:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/SUKIT01.wav", T_VOLUME, T_ATTN); break;
			case 20:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/TERMIN01.wav", T_VOLUME, T_ATTN); break;
			case 21:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/BONVOYAGE.wav", T_VOLUME, T_ATTN); break;
			case 22:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/FROMLASVEGAS.wav", T_VOLUME, T_ATTN); break;
			case 23:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/DUKESENTYA.wav", T_VOLUME, T_ATTN); break;
			case 24:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/MOREWHERECAMEFROM.wav", T_VOLUME, T_ATTN); break;
			case 25:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/damnimgood.wav", T_VOLUME, T_ATTN); break;
			default:
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/killedenemy/VO_MQ_Duke_GranTorino.wav", T_VOLUME, T_ATTN); break;
			}
		}
		return;
	}

	ALERT(at_aiconsole, "[TAUNT] :: DIDN'T WANTED TO TAUNT\n");
}
...
int CBasePlayer::AddPlayerItem( CBasePlayerItem *pItem )
{
	...
	if (pItem->AddToPlayer( this ))
	{
		...
		BOOL isFirstHandgun = FALSE;
#define MAPNAME_LENGHT (unsigned)strlen(MapName)
		char* MapName = (char*)STRING(gpGlobals->mapname);
		if (MAPNAME_LENGHT == 4)
		{
			if ((tolower(MapName[0]) == 'c') && (MapName[1] == '1') && (tolower(MapName[2]) == 'a') && (MapName[3] == '1') && FClassnameIs(pItem->pev, "weapon_9mmhandgun"))
			{
				EMIT_SOUND(ENT(pev), T_CHAN, "player/duke/taunts/special/acrowbar.wav", T_VOLUME, T_ATTN);
				isFirstHandgun = TRUE;
			}
		}

		if (!isFirstHandgun)
			if ( !(FClassnameIs(pItem->pev, "weapon_kick")))
				pev->iuser4 = T_PICKEDUPWEAPON;

		if (FClassnameIs(pItem->pev, "weapon_9mmhandgun") && !(this->pev->weapons & (1 << WEAPON_KICK)))
			this->GiveNamedItem("weapon_kick");
...
void CDeadHEV :: Spawn( void )
{
	...
	DropItem("item_usedkevlar", pev->origin + Vector(0, 0, 20), Vector(0, 0, 0));
...
#define MAPNAME_LENGHT (unsigned)strlen(MapName)
	...
	if (MAPNAME_LENGHT == 5)
	{
		if ((tolower(MapName[0]) == 'c') && (MapName[1] == '2') && (tolower(MapName[2]) == 'a') && (MapName[3] == '3') && (tolower(MapName[4]) == 'd'))
		{
			pPlayer->GiveNamedItem("weapon_kick");
		}
	}
}
