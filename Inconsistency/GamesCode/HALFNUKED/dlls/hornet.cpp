...
LINK_ENTITY_TO_CLASS(blasterprojectile, CBlasterProjectile);

//=========================================================
// Save/Restore
//=========================================================
TYPEDESCRIPTION	CBlasterProjectile::m_SaveData[] =
{
	DEFINE_FIELD(CBlasterProjectile, m_flStopAttack, FIELD_TIME),
	DEFINE_FIELD(CBlasterProjectile, m_iHornetType, FIELD_INTEGER),
	DEFINE_FIELD(CBlasterProjectile, m_flFlySpeed, FIELD_FLOAT),
};

IMPLEMENT_SAVERESTORE(CBlasterProjectile, CBaseMonster);

//=========================================================
// don't let hornets gib, ever.
//=========================================================
int CBlasterProjectile::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	// filter these bits a little.
	bitsDamageType &= ~(DMG_ALWAYSGIB);
	bitsDamageType |= DMG_NEVERGIB;
...
//=========================================================
//=========================================================
void CBlasterProjectile::Spawn(void)
{
	Precache();
	...
	SET_MODEL(ENT(pev), "models/w_blasterprojectile.mdl");
	UTIL_SetSize(pev, Vector(-4, -4, -4), Vector(4, 4, 4));

	SetTouch(&CBlasterProjectile::DieTouch);
	SetThink(&CBlasterProjectile::StartDart);
	...
	if (!FNullEnt(pev->owner) && (pev->owner->v.flags & FL_CLIENT))
	{
		pev->dmg = gSkillData.plrDmgHornet * 3;
	}
	else
	{
		// no real owner, or owner isn't a client. 
		pev->dmg = gSkillData.monDmgHornet * 3;
	}
...
void CBlasterProjectile::Precache()
{
	PRECACHE_MODEL("models/w_blasterprojectile.mdl");

	PRECACHE_SOUND("weapons/blaster_fire.wav");

	PRECACHE_SOUND("weapons/blaster_hit.wav");

	PRECACHE_SOUND("weapons/shrinkray_effect.wav");

	iBlasterPuff = PRECACHE_MODEL("sprites/muz1.spr");
	iBlasterTrail = PRECACHE_MODEL("sprites/laserbeam.spr");
}

//=========================================================
// hornets will never get mad at each other, no matter who the owner is.
//=========================================================
int CBlasterProjectile::IRelationship(CBaseEntity *pTarget)
{
	if (pTarget->pev->modelindex == pev->modelindex)
	{
		return R_NO;
...
// ID's Hornet as their owner
//=========================================================
int CBlasterProjectile::Classify(void)
{
...
// StartDart - starts a hornet out just flying straight.
//=========================================================
void CBlasterProjectile::StartDart(void)
{
	IgniteTrail();

	SetTouch(&CBlasterProjectile::DartTouch);

	SetThink(&CBlasterProjectile::SUB_Remove);
	pev->nextthink = gpGlobals->time + 4;
}

void CBlasterProjectile::IgniteTrail(void)
{
	// trail
	...
	WRITE_BYTE(TE_BEAMFOLLOW);
	WRITE_SHORT(entindex());	// entity
	WRITE_SHORT(iBlasterTrail);	// model
	WRITE_BYTE(1); // life
	WRITE_BYTE(4);  // width
...
void CBlasterProjectile::DartTouch(CBaseEntity *pOther)
{
	DieTouch(pOther);
}

void CBlasterProjectile::DieTouch(CBaseEntity *pOther)
{
	if (pOther && pOther->pev->takedamage)
	{
		pOther->TakeDamage(pev, VARS(pev->owner), pev->dmg, DMG_BULLET);
	...
	EMIT_SOUND(ENT(pev), CHAN_VOICE, "weapons/blaster_hit.wav", 1, 2);
	...
	WRITE_COORD(pev->origin.y);
	WRITE_COORD(pev->origin.z);
	WRITE_SHORT(iBlasterPuff);		// model
