...
void CGib :: SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human )
{
	...
	for ( cSplat = 0 ; cSplat < cGibs ; cSplat++ )
	{
		...
		if ( pevVictim )
		{
			...
			// make the gib fly away from the attack vector
			pGib->pev->velocity = g_vecAttackDir * -0.25;
...
void CBaseMonster :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	// TAUNT STUFF
	string_t playerclass = MAKE_STRING("player");



	if ((pevAttacker->classname == playerclass) && (m_IdealMonsterState != MONSTERSTATE_DEAD))
	{

		//SPECIAL
		if (FClassnameIs(ENT(pev), "monster_gargantua") || FClassnameIs(ENT(pev), "monster_gargantua_emperor"))
		{
			pevAttacker->iuser4 = T_KILLEDBIG;
			ALERT(at_aiconsole, "[TAUNT] :: [%s] was a big trashcan\n", STRING(pev->classname));
		}


		//ENEMY
		else if (Classify() != CLASS_HUMAN_PASSIVE && Classify() != CLASS_PLAYER_ALLY)
		{
			if (iGib == GIB_ALWAYS)
				pevAttacker->iuser4 = T_KILLEDGIBBED;
			else
				pevAttacker->iuser4 = T_KILLEDENEMY;
			ALERT(at_aiconsole, "[TAUNT] :: [%s] was a standard enemy\n", STRING(pev->classname));
		}

		//ALLY
		else
		{
			pevAttacker->iuser4 = T_KILLEDFRIEND;
			ALERT(at_aiconsole, "[TAUNT] :: [%s] was friendly\n", STRING(pev->classname));
		}

		//ALERT(at_aiconsole, "[TAUNT] :: a [%s] was killed by a [%s]\n[TAUNT] :: MESSAGE: %d\n", STRING(pev->classname), STRING(pevAttacker->classname), pevAttacker->iuser4);
	}
	else if (pevAttacker->classname == playerclass)
		ALERT(at_aiconsole, "[TAUNT] :: [%s] was already dead\n", STRING(pev->classname));
	// END OF TAUNT STUFF
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( (pev->flags & FL_MONSTER) && !FNullEnt(pevAttacker) )
	{
		...
		if (bitsDamageType == 1<<24)
		{
			EMIT_SOUND(ENT(pev), CHAN_ITEM, "weapons/shrinkray_effect.wav", 1.0, ATTN_NORM);
			pev->renderfx = kRenderFxShrinked;
