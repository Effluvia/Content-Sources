...
{
	DEFINE_FIELD( CKate, m_iBaseBody, FIELD_INTEGER ), //LRC
...
Schedule_t	slkaFollow[] =
{
	{
		tlkaFollow,
		ARRAYSIZE ( tlkaFollow ),
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slkaFaceTarget[] =
{
	{
		tlkaFaceTarget,
		ARRAYSIZE ( tlkaFaceTarget ),
...
Schedule_t	slIdlekaStand[] =
{
	{ 
		tlIdlekaStand,
		ARRAYSIZE ( tlIdlekaStand ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
...
void CKate :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case KATE_AE_DRAW:
		...
		pev->body = m_iBaseBody + KATE_BODY_GUNDRAWN;
		m_fGunDrawn = TRUE;
		break;

	case KATE_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		pev->body = m_iBaseBody + KATE_BODY_GUNHOLSTERED;
		m_fGunDrawn = FALSE;
		break;
...
void CKate :: Spawn()
{
	...
	pev->body			= m_iBaseBody + KATE_BODY_GUNHOLSTERED; // gun in holster
	m_fGunDrawn			= FALSE;
...
void CKate :: Precache()
{
	...
	PRECACHE_SOUND("barniel/ka_pain1.wav");
	PRECACHE_SOUND("barniel/ka_pain2.wav");
...
void CKate :: TalkInit()
{
	...
	// Kate speech group names (group names are in sentences.txt)
...
void CKate :: PainSound ( void )
{
	...
	{
	case 0: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "barniel/ka_pain1.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	case 1: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "barniel/ka_pain2.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	}
}
...
void CKate :: DeathSound ( void )
{
	switch (RANDOM_LONG(0,2))
	{
	case 0: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "barniel/bn_die1.wav", 1, ATTN_NORM, 0, GetVoicePitch()); break;
	}
}
...
{
	if ( pev->body < m_iBaseBody + KATE_BODY_GUNGONE && !(pev->spawnflags & SF_MONSTER_NO_WPN_DROP))
	{// drop the gun!
		Vector vecGunPos;
		Vector vecGunAngles;

		pev->body = m_iBaseBody + KATE_BODY_GUNGONE;
...
Schedule_t* CKate :: GetScheduleOfType ( int Type )
{
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return slkaFaceTarget;	// override this for different target face behavior
		else
			return psched;
...
void CDeadKate :: Spawn( )
{
	...
	if (pev->sequence == -1)
	{
		ALERT ( at_debug, "Dead Kate with bad pose\n" );
	}
