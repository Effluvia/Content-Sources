...
{
	DEFINE_FIELD( CBarniel, m_iBaseBody, FIELD_INTEGER ), //LRC
...
Schedule_t	slBarFollow[] =
{
	{
		tlBarFollow,
		ARRAYSIZE ( tlBarFollow ),
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slBarFaceTarget[] =
{
	{
		tlBarFaceTarget,
		ARRAYSIZE ( tlBarFaceTarget ),
...
Schedule_t	slIdleBarStand[] =
{
	{ 
		tlIdleBarStand,
		ARRAYSIZE ( tlIdleBarStand ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
...
void CBarniel :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case BARNIEL_AE_DRAW:
		...
		pev->body = m_iBaseBody + BARNIEL_BODY_GUNDRAWN;
		m_fGunDrawn = TRUE;
		break;

	case BARNIEL_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		pev->body = m_iBaseBody + BARNIEL_BODY_GUNHOLSTERED;
		m_fGunDrawn = FALSE;
		break;
...
void CBarniel :: Spawn()
{
	...
	pev->body			= m_iBaseBody + BARNIEL_BODY_GUNHOLSTERED; // gun in holster
	m_fGunDrawn			= FALSE;
...
void CBarniel :: TalkInit()
{
	...
	// Barniel speech group names (group names are in sentences.txt)
...
Schedule_t* CBarniel :: GetScheduleOfType ( int Type )
{
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return slBarFaceTarget;	// override this for different target face behavior
		else
			return psched;
...
void CDeadBarniel :: Spawn( )
{
	...
	if (pev->sequence == -1)
	{
		ALERT ( at_debug, "Dead Barniel with bad pose\n" );
	}
