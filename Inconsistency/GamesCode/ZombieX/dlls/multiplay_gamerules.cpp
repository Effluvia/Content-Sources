...
void CHalfLifeMultiplay :: PlayerSpawn( CBasePlayer *pPlayer )
{
    ...
    if (pPlayer->m_iPlayerClass == 1)
    {
        ...
        g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model", "blue" );
    }
    else if (pPlayer->m_iPlayerClass == 2)
    {
        g_engfuncs.pfnSetClientKeyValue( pPlayer->entindex(),
        g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model", "red" );
    }
    else if (pPlayer->m_iPlayerClass == 3)
    {
        g_engfuncs.pfnSetClientKeyValue( pPlayer->entindex(),
        g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model", "gordon" );
    }
	}
}
