...
****/
//=========================================================
// human scientist Cleansuit (passive lab worker)
//=========================================================
...
class CScientist_suit : public CTalkMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_scientist_suit, CScientist_suit );

TYPEDESCRIPTION	CScientist_suit::m_SaveData[] = 
{
	DEFINE_FIELD( CScientist_suit, m_painTime, FIELD_TIME ),
	DEFINE_FIELD( CScientist_suit, m_healTime, FIELD_TIME ),
	DEFINE_FIELD( CScientist_suit, m_fearTime, FIELD_TIME ),
};

IMPLEMENT_SAVERESTORE( CScientist_suit, CTalkMonster );
...
Schedule_t	sl1FollowScared[] =
{
	{
		tl1FollowScared,
		ARRAYSIZE ( tl1FollowScared ),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE,
...
Schedule_t	sl1FaceTargetScared[] =
{
	{
		tl1FaceTargetScared,
		ARRAYSIZE ( tl1FaceTargetScared ),
		bits_COND_HEAR_SOUND |
		bits_COND_NEW_ENEMY,
		bits_SOUND_DANGER,
		"FaceTargetScared"
...
Task_t	tl1StopFollowing[] =
{
	{ TASK_CANT_FOLLOW,		(float)0 },
};

Schedule_t	sl1StopFollowing[] =
{
	{
		tl1StopFollowing,
		ARRAYSIZE ( tl1StopFollowing ),
		0,
		0,
		"StopFollowing"
	},
...
Schedule_t	sl1FaceTarget[] =
{
	{
		tl1FaceTarget,
		ARRAYSIZE ( tl1FaceTarget ),
...
Schedule_t	sl1IdleSciStand[] =
{
	{ 
		tl1IdleSciStand,
		ARRAYSIZE ( tl1IdleSciStand ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
...
Schedule_t	sl1ScientistCover[] =
{
	{ 
		tl1ScientistCover,
		ARRAYSIZE ( tl1ScientistCover ), 
		bits_COND_NEW_ENEMY,
		0,
		"ScientistCover"
	},
...
Schedule_t	sl1ScientistHide[] =
{
	{ 
		tl1ScientistHide,
		ARRAYSIZE ( tl1ScientistHide ), 
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
...
Schedule_t	sl1ScientistStartle[] =
{
	{ 
		tl1ScientistStartle,
		ARRAYSIZE ( tl1ScientistStartle ), 
		bits_COND_NEW_ENEMY |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
		bits_COND_SEE_FEAR |
...
DEFINE_CUSTOM_SCHEDULES( CScientist_suit )
{
	sl1Follow,
	sl1FaceTarget,
	sl1IdleSciStand,
...
IMPLEMENT_CUSTOM_SCHEDULES( CScientist_suit, CTalkMonster );


void CScientist_suit::DeclineFollowing( void )
{
	Talk( 10 );
...
void CScientist_suit :: Scream( void )
{
	if ( FOkToSpeak() )
	{
		Talk( 10 );
...
Activity CScientist_suit::GetStoppedActivity( void )
{ 
	if ( m_hEnemy != NULL ) 
		return ACT_EXCITED;
...
void CScientist_suit :: StartTask( Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_SAY_HEAL:
...
void CScientist_suit :: RunTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_RUN_PATH_SCARED:
...
// relationship table.
//=========================================================
int	CScientist_suit :: Classify ( void )
{
...
//=========================================================
void CScientist_suit :: SetYawSpeed ( void )
{
	int ys;
...
//=========================================================
void CScientist_suit :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{		
...
// Spawn
//=========================================================
void CScientist_suit :: Spawn( void )
{
	Precache( );
	...
	if (pev->health == 0)
		pev->health			= gSkillData.scientistsuitHealth;
...
//	m_flDistTooFar		= 256.0;
	...
	MonsterInit();
	SetUse(&CScientist_suit :: FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CScientist_suit :: Precache( void )
{
	if (pev->model)
...
void CScientist_suit :: TalkInit()
{
	...
	m_szFriends[3] = "monster_barney_suit";
	m_szFriends[4] = "monster_barniel";
...
int CScientist_suit :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
...
//=========================================================
int CScientist_suit :: ISoundMask ( void )
{
...
// PainSound
//=========================================================
void CScientist_suit :: PainSound ( void )
{
...
// DeathSound 
//=========================================================
void CScientist_suit :: DeathSound ( void )
{
	PainSound();
}


void CScientist_suit::Killed( entvars_t *pevAttacker, int iGib )
{
	SetUse( NULL );	
...
void CScientist_suit :: SetActivity ( Activity newActivity )
{
	int	iSequence;
...
Schedule_t* CScientist_suit :: GetScheduleOfType ( int Type )
{
	Schedule_t *psched;
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return sl1FaceTarget;	// override this for different target face behavior
		else
			return psched;
	...
	case SCHED_CANT_FOLLOW:
		return sl1StopFollowing;

	case SCHED_PANIC:
		return sl1SciPanic;

	case SCHED_TARGET_CHASE_SCARED:
		return sl1FollowScared;

	case SCHED_TARGET_FACE_SCARED:
		return sl1FaceTargetScared;
	...
	case SCHED_IDLE_STAND:
		...
		if (psched == slIdleStand)
			return sl1IdleSciStand;
		else
			return psched;

	case SCHED_HIDE:
		return sl1ScientistHide;

	case SCHED_STARTLE:
		return sl1ScientistStartle;
...
Schedule_t *CScientist_suit :: GetSchedule ( void )
{
	...
	case MONSTERSTATE_COMBAT:
		if ( HasConditions( bits_COND_NEW_ENEMY ) )
			return sl1Fear;					// Point and scream!
		if ( HasConditions( bits_COND_SEE_ENEMY ) )
			return sl1ScientistCover;		// Take Cover
		
		if ( HasConditions( bits_COND_HEAR_SOUND ) )
			return slTakeCoverFromBestSound;	// Cower and panic from the scary sound!

		return sl1ScientistCover;			// Run & Cower
		break;
	}
...
MONSTERSTATE CScientist_suit :: GetIdealState ( void )
{
	switch ( m_MonsterState )
	{
	case MONSTERSTATE_ALERT:
...
void CScientist_suit::Heal( void )
{
	if ( !CanHeal() )
		return;
...
int CScientist_suit::FriendNumber( int arrayNumber )
{
...
// Dead Scientist PROP
//=========================================================
class CDeadScientist_suit : public CBaseMonster
{
public:
	void Spawn( void );
...
char *CDeadScientist_suit::m_szPoses[] = { "lying_on_back", "lying_on_stomach", "dead_sitting", "dead_hang", "dead_table1", "dead_table2", "dead_table3" };

void CDeadScientist_suit::KeyValue( KeyValueData *pkvd )
{
...
LINK_ENTITY_TO_CLASS( monster_scientist_dead_suit, CDeadScientist_suit );

//
// ********** DeadScientist SPAWN **********
//
void CDeadScientist_suit :: Spawn( )
{
	PRECACHE_MODEL("models/cleansuit_scientist.mdl");
	SET_MODEL(ENT(pev), "models/cleansuit_scientist.mdl");
...
class CSittingScientist_suit : public CScientist_suit // kdb: changed from public CBaseMonster so he can speak
{
public:
	void Spawn( void );
	void  Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_sitting_scientist_suit, CSittingScientist_suit );
TYPEDESCRIPTION	CSittingScientist_suit::m_SaveData[] = 
{
	// Don't need to save/restore m_baseSequence (recalced)
	DEFINE_FIELD( CSittingScientist_suit, m_headTurn, FIELD_INTEGER ),
	DEFINE_FIELD( CSittingScientist_suit, m_flResponseDelay, FIELD_FLOAT ),
};

IMPLEMENT_SAVERESTORE( CSittingScientist_suit, CScientist_suit );
...
// ********** Scientist SPAWN **********
//
void CSittingScientist_suit :: Spawn( )
{
	if (pev->model)
		PRECACHE_MODEL((char*)STRING(pev->model)); //LRC
	else
	...
	SetThink(&CSittingScientist_suit ::SittingThink);
	SetNextThink( 0.1 );

	DROP_TO_FLOOR ( ENT(pev) );
}

void CSittingScientist_suit :: Precache( void )
{
	m_baseSequence = LookupSequence( "sitlookleft" );
	TalkInit();
}
...
//=========================================================
int	CSittingScientist_suit :: Classify ( void )
{
	return m_iClass?m_iClass:CLASS_HUMAN_PASSIVE;
}


int CSittingScientist_suit::FriendNumber( int arrayNumber )
{
...
// sit, do stuff
//=========================================================
void CSittingScientist_suit :: SittingThink( void )
{
	CBaseEntity *pent;	
...
void CSittingScientist_suit :: SetAnswerQuestion( CTalkMonster *pSpeaker )
{
...
//=========================================================
int CSittingScientist_suit :: FIdleSpeak ( void )
{ 
