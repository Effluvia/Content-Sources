...
void CStudioModelRenderer::Init( void )
{
	...
	m_pCvarDrawShadows		= CVAR_CREATE( "r_shadows", "2", FCVAR_ARCHIVE );
...
void CStudioModelRenderer::StudioSetUpTransform (int trivial_accept)
{
	...
	cl_entity_t*	pentity;
	if( m_pCurrentEntity->curstate.effects & EF_SYNC_FULL && m_pCurrentEntity->curstate.owner )
	{
		int parententindex = m_pCurrentEntity->curstate.owner;
		pentity = gEngfuncs.GetEntityByIndex( parententindex );
	}
	else
	{
	...
	if (pentity->curstate.movetype == MOVETYPE_STEP) 
	{
		...
		for (i = 0; i < 3; i++)
		{
			modelpos[i] += (pentity->origin[i] - pentity->latched.prevorigin[i]) * f;
		}
...
void CStudioModelRenderer::StudioSetupShadows( void )
{
	...
	{
		// Determine the shadow's height
		static pmtrace_t traceResult;
		gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );
		gEngfuncs.pEventAPI->EV_PlayerTrace( m_pCurrentEntity->origin, m_pCurrentEntity->origin - Vector(0, 0, 4096), PM_WORLD_ONLY, -1, &traceResult );

		m_flShadowHeight = traceResult.endpos.z;
	}
}
...
void CStudioModelRenderer::GL_StudioDrawShadow( void )
{
	...
	glDepthFunc(GL_LESS);
	glDisable(GL_TEXTURE_2D);
	glColor4f(GL_ZERO, GL_ZERO, GL_ZERO, GL_ONE);
...
{
	// Get elight list
	gELightList.GetLightList(m_pCurrentEntity->origin, m_vMins, m_vMaxs, m_pEntityLights, &m_iNumEntityLights);
