...
void CGib :: SpawnRandomGibs( entvars_t *pevVictim, int cGibs, int human )
{
	...
	for ( cSplat = 0 ; cSplat < cGibs ; cSplat++ )
	{
		...
		if ( pevVictim )
		{
			...
			pGib->pev->velocity = pGib->pev->velocity * RANDOM_FLOAT ( 700, 900 );
...
void CGib :: Spawn( const char *szGibModel )
{
	...
	pev->friction = 0.8; // deading the bounce a bit
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if( bitsDamageType & DMG_BULLET )
	{
		...
		if(pAttacker->IsPlayer())
		{
			...
			{
				case 0: EMIT_SOUND_DYN(ENT(pevAttacker), CHAN_BODY, "weapons/bullet_hit1.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 98 + RANDOM_LONG(0,7)); break;
				case 1: EMIT_SOUND_DYN(ENT(pevAttacker), CHAN_BODY, "weapons/bullet_hit2.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 98 + RANDOM_LONG(0,7)); break;
			}
		}
	}
...
void CBaseEntity::TraceAttack(entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if ( pev->takedamage )
	{
		...
		if ( blood != DONT_BLEED )
		{
			SpawnBlood( vecOrigin, blood, flDamage, bitsDamageType );// a little surface blood.
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			case BULLET_PLAYER_762:		
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmg762, vecDir, &tr, (DMG_BULLET|DMG_SNIPER)); 
				break;
