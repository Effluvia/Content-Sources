...
public:
	...
	virtual BOOL	CanSuck( void ) { return FALSE; }
...
public:
	...
	virtual float StudioFrameAdvance( float flInterval = 0.0 ); // accumulate animation frame time from last time called until now
	...
	virtual void DispatchAnimEvents ( float flFutureInterval = 0.1 ); // Handle events that have happend since last time called up until X seconds into the future
	...
	virtual void GetAutomovement( Vector &origin, Vector &angles, float flInterval = 0.1 );
	virtual int  FindTransition( int iEndingSequence, int iGoalSequence, int *piDir );
	...
	virtual int ExtractBbox( int sequence, float *mins, float *maxs );
	virtual void SetSequenceBox( void );
...
#define DMG_SNIPER			(1 << 24)	// Shot by the m40a1
#define DMG_NAIL			(1 << 25)	// Shot by the nailgun

// these are the damage types that are allowed to gib corpses
#define DMG_GIB_CORPSE		( DMG_CRUSH | DMG_FALL | DMG_BLAST | DMG_SONIC | DMG_CLUB | DMG_NAIL )
