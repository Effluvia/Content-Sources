...
// Entity light
//=========================================================
extern int gmsgELight;
class CEnvELight : public CBaseEntity
{
public:
	void	Spawn( void );
...
void CEnvELight::SendInitMessages ( CBaseEntity* pPlayer )
{
...
void CEnvWeather::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "impactpsrite"))
	{
...
void CEnvWeather::Spawn( void )
{
	...
	if( FStringNull(pev->targetname)
		|| FBitSet(pev->spawnflags, SF_WATHER_ACTIVE) )
		m_bActive = TRUE;
}

void CEnvWeather::Precache( void )
{
	m_iParticleSprite = PRECACHE_MODEL((char *)STRING(m_sParticleSprite));
	m_iImpactSprite = PRECACHE_MODEL((char *)STRING(m_sImpactSprite));
}
...
void CEnvWeather::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	MESSAGE_BEGIN(MSG_ONE, gmsgWeather, NULL, pPlayer->pev);
		...
		if(m_bActive)
		{
			...
			WRITE_COORD( m_iWindX );
			WRITE_COORD( m_iWindY );
			WRITE_SHORT( m_iParticleSprite );
...
// env_spritetrain
//=========================================================
class CModelTrain : public CBasePlatTrain
{
public:
	void Spawn( void );
	void Precache( void );
...
{
	DEFINE_FIELD( CModelTrain, m_pevCurrentTarget, FIELD_EVARS ),
	DEFINE_FIELD( CModelTrain, m_activated, FIELD_BOOLEAN ),
};

IMPLEMENT_SAVERESTORE( CModelTrain, CBasePlatTrain );
...
void CModelTrain :: Wait( void )
{
	...
	{
		if ( !(pev->spawnflags & SF_TRAIN_NO_PASS_TRIGGER) )
		{
...
void CModelTrain :: Next( void )
{
	...
	if( m_pevCurrentTarget && m_pevCurrentTarget->avelocity.Length() && pev->avelocity.Length() )
	{
		pev->avelocity = m_pevCurrentTarget->avelocity;
		ALERT( at_aiconsole, "Train %s avelocity to %4.2f %4.2f %4.2f\n", STRING(pev->targetname), pev->avelocity.x, pev->avelocity.y, pev->avelocity.z );
	}
	...
	if(FBitSet(m_pevCurrentTarget->spawnflags, SF_CORNER_TELEPORT))
	{
		...
		UTIL_SetOrigin( pev, pTarg->pev->origin );
...
void CModelTrain :: Activate( void )
{
	...
	if ( !m_activated )
	{
		...
		UTIL_SetOrigin( pev, pevTarg->origin );
		
		if ( FStringNull(pev->targetname) || (pev->spawnflags & SF_FUNCTRAIN_START_ON))
		{	// not triggered, so start immediately
			pev->nextthink = pev->ltime + 0.1;
			SetThink( &CModelTrain::Next );
			pev->spawnflags &= ~SF_FUNCTRAIN_START_ON;
		}
		else
			pev->spawnflags |= SF_TRAIN_WAIT_RETRIGGER;
	}
...
void CModelTrain::OverrideReset( void )
{
	CBaseEntity	*pTarg;
...
// Generic Item
//=========================================================
extern int gmsgModelCamera;
class CModelCamera : public CBaseEntity
{
public:
	void Spawn( void );
	void Activate( void );
	void KeyValue( KeyValueData *pkvd );
	void ModelCamera( void );
	void SendMessage( void );
	void SendInitMessages ( CBaseEntity* pPlayer );
...
private:
	...
	float m_flMouseTimeout;
	float m_flMouseMaxX;
	float m_flMouseMaxY;

	edict_t* m_pTargetEntity;
};

TYPEDESCRIPTION	CModelCamera::m_SaveData[] = 
{
	DEFINE_FIELD( CModelCamera, m_bActive, FIELD_BOOLEAN ),
	DEFINE_FIELD( CModelCamera, m_iAttachment1, FIELD_INTEGER ),
	DEFINE_FIELD( CModelCamera, m_iAttachment2, FIELD_INTEGER ),
	DEFINE_FIELD( CModelCamera, m_iAttachment3, FIELD_INTEGER ),
	DEFINE_FIELD( CModelCamera, m_flMouseTimeout, FIELD_FLOAT ),
	DEFINE_FIELD( CModelCamera, m_flMouseMaxX, FIELD_FLOAT ),
	DEFINE_FIELD( CModelCamera, m_flMouseMaxY, FIELD_FLOAT ),
	DEFINE_FIELD( CModelCamera, m_pTargetEntity, FIELD_EDICT )
};

IMPLEMENT_SAVERESTORE( CModelCamera, CBaseEntity );
LINK_ENTITY_TO_CLASS( env_modelcamera, CModelCamera );

void CModelCamera::KeyValue( KeyValueData *pkvd )
{
	if (FStrEq(pkvd->szKeyName, "attach1"))
	{
		m_iAttachment1 = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "attach2"))
	{
		m_iAttachment2 = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "attach3"))
	{
		m_iAttachment3 = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "timeout"))
	{
		m_flMouseTimeout = atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "mousemax_x"))
	{
		m_flMouseMaxX = atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "mousemax_y"))
	{
		m_flMouseMaxY = atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
		CBaseEntity::KeyValue( pkvd );
...
void CModelCamera :: Activate ( void )
{
	edict_t* pEdict = FIND_ENTITY_BY_TARGETNAME( NULL, STRING(pev->target) );
	if( FNullEnt( pEdict ) )
	{
		ALERT( at_console, "Error: model camera can't find target %s\n", STRING(pev->target) );
		return;
	}

	m_pTargetEntity = pEdict;
}

void CModelCamera::SendMessage( void )
{
	CBasePlayer *pPlayer = (CBasePlayer *)CBaseEntity::Instance( g_engfuncs.pfnPEntityOfEntIndex( 1 ) );

	MESSAGE_BEGIN( MSG_ALL, gmsgModelCamera, NULL );
	if( m_bActive )
	{
		WRITE_SHORT( ENTINDEX( m_pTargetEntity ) );
		WRITE_BYTE( m_iAttachment1 );
		WRITE_BYTE( m_iAttachment2 );
		WRITE_BYTE( m_iAttachment3 );
		WRITE_COORD( m_flMouseTimeout );
		WRITE_COORD( m_flMouseMaxX );
		WRITE_COORD( m_flMouseMaxY );

		pPlayer->SetViewEntity( m_pTargetEntity, m_iAttachment1 );
	...
	else
	{
		...
		pPlayer->SetViewEntity( NULL, 0 );
		pPlayer->EnableControl( TRUE );
	}
	MESSAGE_END();
}

void CModelCamera::SendInitMessages ( CBaseEntity* pPlayer )
{
	if(!m_bActive)
		return;
...
// Quake 1 model
//=========================================================
LINK_ENTITY_TO_CLASS( env_q1model, CEnvQ1Model );

void CEnvQ1Model :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	pev->sequence++;
	if(pev->sequence == m_extraData.numsequences)
		pev->sequence = 0;

	alias_sequence_t* psequence = &m_extraData.sequences[pev->sequence];
	pev->frame = 0;
}

void CEnvQ1Model :: Precache ( void )
{
	PRECACHE_MODEL( (char *)STRING(pev->model) );
}

void CEnvQ1Model::Spawn( void )
{
	Precache();
	SET_MODEL( ENT(pev), STRING(pev->model));
	...
	// Quake 1 models animate at 10 fps static
	pev->framerate = 1.0f;

	SetThink(&CEnvQ1Model::Animate);
	pev->nextthink = gpGlobals->time + 0.1;
...
void CEnvQ1Model::MapSequences( void )
{
	...
	Alias_GetSequenceInfo((aliashdr_t*)pmodel, &m_extraData);
	Alias_LoadSequenceInfo(STRING(pev->model), szgamedir, &m_extraData);
}

void CEnvQ1Model::Animate( void )
{
	...
	// Allow an extra frame for wrapping around with looped animations
	int numframes = psequence->looped ? (psequence->numframes+1) : psequence->numframes;

	float framerate;
	if (psequence->numframes > 1)
		framerate = 256 * psequence->fps / (numframes - 1);
	else
		framerate = 256;

	pev->frame += flInterval * framerate * pev->framerate;
	pev->animtime = gpGlobals->time;
...
#if 0
//=========================================================
// Maximillian's energy ball
//=========================================================
#define MAX_BALL_BEAMS 16
class CEnvMaximillianBall : public CBaseEntity
{
public:
	void	Spawn( void );
	void	Precache( void );
...
public:
	CBeam*	m_pBeams[MAX_BALL_BEAMS];
	int		m_iNumBeams;
};

TYPEDESCRIPTION	CEnvMaximillianBall::m_SaveData[] = 
{
	DEFINE_ARRAY( CEnvMaximillianBall, m_pBeams, FIELD_CLASSPTR, MAX_BALL_BEAMS ),
	DEFINE_FIELD( CEnvMaximillianBall, m_iNumBeams, FIELD_INTEGER )
};

IMPLEMENT_SAVERESTORE( CEnvMaximillianBall, CBaseEntity );
LINK_ENTITY_TO_CLASS( env_energyball, CEnvMaximillianBall );

void CEnvMaximillianBall :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
}

void CEnvMaximillianBall :: Precache ( void )
{
	PRECACHE_MODEL( "sprites/cnt1.spr" );
}

void CEnvMaximillianBall::Spawn( void )
{
	Precache();
	...
	SetThink( &CEnvMaximillianBall::HoverThink );
	SetTouch( &CEnvMaximillianBall::BounceTouch );
	pev->nextthink = gpGlobals->time + 0.1;
}

void CEnvMaximillianBall::HoverThink( void )
{
	// Get a random angle that hits something
	while(1)
	{
		Vector vecAngle;
		vecAngle[0] = RANDOM_LONG(0, 360);
		vecAngle[1] = RANDOM_LONG(0, 360);
		vecAngle[2] = RANDOM_LONG(0, 360);

		UTIL_MakeAimVectors( vecAngle );

		TraceResult tr;
		UTIL_TraceLine ( pev->origin, pev->origin + gpGlobals->v_forward * 512, dont_ignore_monsters, ENT( pev ), &tr);

		if(tr.flFraction != 1.0 && UTIL_PointContents(tr.vecEndPos) != CONTENTS_SKY)
		{
			// Remove if it's already there
			if(m_pBeam[m_iNumBeams])
			{
				UTIL_Remove(m_pBeam[m_iNumBeams]);
				m_pBeam[m_iNumBeams] = NULL;
			}

			m_pBeam[m_iNumBeams] = CBeam::BeamCreate( "sprites/lgtning.spr", 30 );
			if (!m_pBeam[m_iNumBeams])
				return;

			m_pBeam[m_iNumBeams]->PointEntInit( tr.vecEndPos, entindex( ) );
			m_pBeam[m_iNumBeams]->SetColor( 96, 128, 16 );
			m_pBeam[m_iNumBeams]->SetBrightness( 64 );
			m_pBeam[m_iNumBeams]->SetNoise( 80 );
			m_iNumBeams++;

			if(m_iNumBeams >= MAX_BALL_BEAMS)
				m_iNumBeams = 0;
	...
	// Set frame and nextthink
	pev->frame += (int)(pev->frame + 1) % 4;
	pev->nextthink = gpGlobals->time + 0.1;
}

void CEnvMaximillianBall::BounceTouch( CBaseEntity *pOther )
{
...
// env_parenter
//=========================================================
#define MAX_SYNC_ENTITIES	32
#define SF_ENVSYNC_STARTON	1

class CEnvSync : public CBaseEntity
{
public:
	void Spawn( void );
	void PreFrame( void );
...
public:
	EHANDLE m_syncEntities[MAX_SYNC_ENTITIES];
	int m_numSyncEntities;

	EHANDLE m_syncTarget;
	BOOL m_bActive;
};

TYPEDESCRIPTION	CEnvSync::m_SaveData[] = 
{
	DEFINE_ARRAY( CEnvSync, m_syncEntities, FIELD_EHANDLE, MAX_SYNC_ENTITIES ),
	DEFINE_FIELD( CEnvSync, m_numSyncEntities, FIELD_INTEGER ),
	DEFINE_FIELD( CEnvSync, m_syncTarget, FIELD_EHANDLE ),
	DEFINE_FIELD( CEnvSync, m_bActive, FIELD_BOOLEAN )
};

IMPLEMENT_SAVERESTORE( CEnvSync, CBaseEntity );
LINK_ENTITY_TO_CLASS( env_sync, CEnvSync );

void CEnvSync::Spawn( void )
{
	Precache();
...
void CEnvSync::Activate( void )
{
	if(m_bActive)
		return;

	if(FStringNull(pev->targetname) || FBitSet(pev->spawnflags, SF_ENVSYNC_STARTON))
		Use(this, this, USE_TOGGLE, 0);
}

void CEnvSync::PreFrame( void )
{
	if(!m_bActive)
		return;

	for(int i = 0; i < m_numSyncEntities; i++)
	{
		if(!m_syncEntities[i])
			continue;

		UTIL_SetOrigin(m_syncEntities[i]->pev, m_syncTarget->pev->origin);
		m_syncEntities[i]->pev->angles = m_syncTarget->pev->angles;
	}
}

void CEnvSync :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if(FStringNull(pev->target))
	{
		ALERT(at_console, "Error: null target on env_sync entity %s\n", STRING(pev->targetname));
		return;
	}

	if(FStringNull(pev->netname))
	{
		ALERT(at_console, "Error: null sync entity on env_sync entity %s\n", STRING(pev->netname));
		return;
	}
	...
	if(!m_syncTarget)
	{
		edict_t* pParent = FIND_ENTITY_BY_TARGETNAME(NULL, STRING(pev->netname));
		if(!pParent)
		{
			ALERT(at_console, "Error: sync target %s not found for env_sync entity %s\n", STRING(pev->netname), STRING(pev->targetname));
			return;
		}

		m_syncTarget = CBaseEntity::Instance(pParent);
	}

	if(!m_numSyncEntities)
	{
		...
		for (;;)
		{
			...
			m_syncEntities[m_numSyncEntities] = CBaseEntity::Instance(pentTarget);
			m_numSyncEntities++;
		}

		if(!m_numSyncEntities)
		{
			ALERT(at_console, "Error: no entities found for env_sync entity %s with the name %s\n", STRING(pev->targetname), STRING(pev->target));
			return;
		}
	}
	...
	for(int i = 0; i < m_numSyncEntities; i++)
	{
		EHANDLE pEntity = m_syncEntities[i];

		if(m_bActive)
		{
			m_syncEntities[i]->pev->effects |= EF_SYNC_FULL;
			m_syncEntities[i]->pev->owner = m_syncTarget->edict();
		...
		{
			m_syncEntities[i]->pev->effects &= ~EF_SYNC_FULL;
			m_syncEntities[i]->pev->owner = NULL;
		}
	}
}