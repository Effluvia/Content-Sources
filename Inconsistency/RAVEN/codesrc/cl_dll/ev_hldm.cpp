...
void EV_FireQ1Shotgun( struct event_args_s *args );
...
float EV_HLDM_PlayTextureSound( int idx, pmtrace_t *ptr, float *vecSrc, float *vecEnd, int iBulletType )
{
	...
	case CHAR_TEX_CONCRETE: fvol = 0.9;	fvolbar = 0.6;
		...
		cnt = 2;
		break;
	case CHAR_TEX_SNOW: fvol = 0.9;	fvolbar = 0.6;
...
void EV_AliasWeaponAnimation( int sequence )
{
	cl_entity_t* pview = gEngfuncs.GetViewModel();
	if(!pview)
		return;

	pview->curstate.sequence = sequence;
	pview->curstate.framerate = 1.0;
	pview->curstate.frame = 0.0;

	pview->curstate.animtime = gEngfuncs.GetClientTime();

	pview->latched.sequencetime = 0;
	pview->latched.prevsequence = 0;
	pview->latched.prevanimtime = 0;
}
...
void EV_FireEagle( event_args_t *args )
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? EAGLE_SHOOT_EMPTY : EAGLE_SHOOT, 2 );

		V_PunchAxis( 0, spotOn ? -2.0 : -4.0 );
	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/desert_fire.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
...
void EV_FireM40A1( event_args_t *args )
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? SNIPER_FIRE_LAST : SNIPER_FIRE, 2 );
...
//	    DISPLACER START
//======================
void EV_MakeGunBeams( int entindex, float noiseScale )
{
	int iAttach = 0;
	int mdlIndex = gEngfuncs.pEventAPI->EV_FindModelIndex( "sprites/lgtning.spr" );
	...
	for( size_t uiIndex = 0; uiIndex < 4; ++uiIndex )
	{
		...
		gEngfuncs.pEfxAPI->R_BeamEnts(
			entindex | ( iStartAttach << 12 ), entindex | ( iEndAttach << 12 ),
			mdlIndex,
			1,
			1, 50 * 0.01 + noiseScale * 4 * 0.01, 255 / 255.0, 60, 0, 10,
			96 / 255.0, 96 / 255.0, 128 / 255.0 );
	}
}

void EV_MakeCentralBeam( int entindex, int blackholeindex, float noiseScale )
{
	int iStartAttach = 1;
	int mdlIndex = gEngfuncs.pEventAPI->EV_FindModelIndex( "sprites/laserbeam.spr" );

	gEngfuncs.pEfxAPI->R_BeamEnts(
		blackholeindex, entindex | ( iStartAttach << 12 ),
		mdlIndex,
		1,
		4, (noiseScale - 1)* 2 * 0.01, 255 / 255.0, 10, 0, 10,
		96 / 255.0, 96 / 255.0, 128 / 255.0 );
}

void EV_MakeBlackHoleBeams( int entindex, int blackholeindex, float noiseScale )
{
	int iAttach = 0;
	int mdlIndex = gEngfuncs.pEventAPI->EV_FindModelIndex( "sprites/lgtning.spr" );
	...
	for( size_t uiIndex = 0; uiIndex < 4; ++uiIndex )
	{
		...
		gEngfuncs.pEfxAPI->R_BeamEnts(
			entindex | ( iStartAttach << 12 ), blackholeindex,
			mdlIndex,
			1,
			2, 2 * 0.01 + noiseScale * 2 * 0.01, 255 / 255.0, 10, 0, 10,
			96 / 255.0, 96 / 255.0, 128 / 255.0 );
	}
}
...
void EV_Displacer( event_args_t *args )
{
	...
	int state = args->iparam1;
	int bhindex = args->iparam2;
	float noise = args->fparam1;
	noise = (noise/0.25)*1;

	vec3_t origin;
	VectorCopy( args->origin, origin );

	EV_MakeGunBeams( idx, noise );

	switch( state )
	{
	case DISPLACER_STATE_SPINUP:
		{
			if( EV_IsLocal( args->entindex ) )
				gEngfuncs.pEventAPI->EV_WeaponAnimation( DISPLACER_SPINUP, 0 );

			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/displacer_spin.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, PITCH_NORM );
		}
		break;
	case DISPLACER_STATE_SPIN:
		{
			if( EV_IsLocal( args->entindex ) )
				gEngfuncs.pEventAPI->EV_WeaponAnimation( DISPLACER_SPIN, 0 );

			EV_MakeCentralBeam( idx, bhindex, noise );
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/displacer_spin2.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, PITCH_NORM );
		}
		break;
	case DISPLACER_STATE_FIRE:
		{
			if( EV_IsLocal( args->entindex ) )
			{
			...
			EV_MakeBlackHoleBeams( idx, bhindex, noise );
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/displacer_fire.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, PITCH_NORM );
		}
		break;
	}
}
...
//	  Q1 SHOTGUN START
//======================
void EV_FireQ1Shotgun( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	AngleVectors( angles, forward, right, up );
	cl_entity_t* pentity = gEngfuncs.GetViewModel();

	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		EV_AliasWeaponAnimation( Q1SHOTGUN_FIRE );

		V_PunchAxis( 0, -2.0 );
	}

	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/sattck1.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
...
void EV_FireRifle( event_args_t *args )
{
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( RIFLE_FIRE1 + gEngfuncs.pfnRandomLong(0,2), 2 );
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/srifle1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/srifle2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 2:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/srifle3.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_RIFLE, 2, &tracerCount[idx-1], args->fparam1, args->fparam2 );
}
//======================
//		 RIFLE END
//======================
...
void EV_FireNailgun( event_args_t *args )
{
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/nailgun_fire1.wav", 1, ATTN_NORM, 0, 100 );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/nailgun_fire2.wav", 1, ATTN_NORM, 0, 100 );
		break;
	case 2:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/nailgun_fire3.wav", 1, ATTN_NORM, 0, 100 );
		break;
	}
}
//======================
...
void EV_FireSAW( event_args_t *args )
{
	...
	if( linkToggle )
		...
		shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/saw_link.mdl");// brass shell
	...
	if ( EV_IsLocal( idx ) )
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( SAW_FIRE1 + gEngfuncs.pfnRandomLong(0,2), body );

		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -2, 2 ) );
		V_PunchAxis( 1, gEngfuncs.pfnRandomFloat( -1, 1 ) );
		V_PunchAxis( 2, gEngfuncs.pfnRandomFloat( -2, 2 ) );
	}
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_shoot1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_shoot2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 2:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_shoot3.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
