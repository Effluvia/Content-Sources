...
//MODDD - SWEET GOD THAT MUCH Alloca.
//MODDD - avoid alloca
// (just to get the same comment for looking up later)
// MASS REPLACE:  any mentions of '_alloca' replaced with "CUSTOMALLOC".
// And then those lead to 'malloc':
...
CBaseFileSystem* CSearchPath::m_fs = 0;
...
void CBaseFileSystem::AddMapPackFile( const char *pPath, SearchPathAdd_t addType )
{
	...
	newPath = ( char * )CUSTOMALLOC( strlen( pPath ) + 2 );
...
// Purpose:
//-----------------------------------------------------------------------------
CSearchPath *CBaseFileSystem::FindWritePath( const char *pathID )
{
...
FileHandle_t CBaseFileSystem::FindFile( const CSearchPath *path, const char *pFileName, const char *pOptions )
{
	...
	if ( path->m_bIsPackFile )
	{
		...
		char *temp = (char *)CUSTOMALLOC( strlen( pFileName ) + 1 );
	...
	else
	{
		...
		if ( strchr( pFileName, ':' ) )
		{
			pTmpFileName = ( char * )CUSTOMALLOC( strlen( pFileName ) + 1 );
			strcpy( pTmpFileName, pFileName );
		}
		else
		{
			pTmpFileName = ( char * )CUSTOMALLOC( strlen( path->GetPathString() ) + strlen( pFileName ) + 1 );
		...
		if( fp )
		{
			if( m_pLogFile )
			{
				...
				char *tmp = ( char * )CUSTOMALLOC( strlen( pTmpFileName ) + 1 );
...
void CBaseFileSystem::Close( FileHandle_t file )
{
	...
	CFileHandle *fh_uhhh = ( CFileHandle *)file;
	...
	int ecks = this->m_PackFileHandles.Size();
	if(ecks < 1){
		// ???
		return;
	}
	...
	*/
	...
	{
		//MODDD - FUCK THIS SHIT
		Trace_FClose( fh->m_pFile );
		//CBaseFileSystem::Trace_FClose( fh->m_pFile );
...
bool CBaseFileSystem::IsFileWritable( char const *pFileName, char const *pPathID /*=0*/ )
{
	...
	for( i = 0; i < m_SearchPaths.Size(); i++ )
	{
		...
		char *pTmpFileName;
		pTmpFileName = ( char * )CUSTOMALLOC( strlen( m_SearchPaths[i].GetPathString() ) + strlen( pFileName ) + 1 );
...
const char *CBaseFileSystem::FindFirst( const char *pWildCard, FileFindHandle_t *pHandle )
{
			...
			pFindData->currentSearchPathID++ )
	{
		...
		pTmpFileName = ( char * )CUSTOMALLOC( size + 1 ); // don't need two NULL terminators.
										   ...
										   int maxSearchPathID )
{
	...
	char *pFileNameWithPath = ( char * )CUSTOMALLOC( pathStrLen + fileNameStrLen + 1 );
	...
	for( i = minSearchPathID; i <= maxSearchPathID; i++ )
	{
		...
		fullFilePath = ( char * )CUSTOMALLOC( len + 1 );
...
bool CBaseFileSystem::FindNextFileHelper( FindData_t *pFindData )
{
	...
	while( pFindData->currentSearchPathID < m_SearchPaths.Count() ) 
	{
		...
		char *pTmpFileName;
		pTmpFileName = ( char * )CUSTOMALLOC( strlen( m_SearchPaths[pFindData->currentSearchPathID].GetPathString() ) + 
...
int CBaseFileSystem::GetLocalPathLen( const char *pFileName )
{
	...
	for( i = 0; i < m_SearchPaths.Count(); i++ )
	{
		...
		pTmpFileName = ( char * )CUSTOMALLOC( len + 1 );
		strcpy( pTmpFileName, m_SearchPaths[i].GetPathString() );
		strcat( pTmpFileName, pFileName );
		FixSlashes( pTmpFileName );
		if( FS_stat( pTmpFileName, &buf ) != -1 )
...
bool CBaseFileSystem::FullPathToRelativePath( const char *pFullpath, char *pRelative )
{
	...
	char *inpath = (char *)CUSTOMALLOC( inlen + 1 );
	Assert( inpath );
...
// Input  : src - 
//-----------------------------------------------------------------------------
COpenedFile::COpenedFile( const COpenedFile& src )
{
	m_pFile = src.m_pFile;
	if ( src.m_pName )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool COpenedFile::operator==( const COpenedFile& src ) const
{
...
// Input  : *name - 
//-----------------------------------------------------------------------------
void COpenedFile::SetName( char const *name )
{
	delete[] m_pName;
...
// Output : char
//-----------------------------------------------------------------------------
char const *COpenedFile::GetName( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSearchPath::PackFileLessFunc( CPackFileEntry const& src1, CPackFileEntry const& src2 )
{
	return ( src1.m_Name < src2.m_Name );
}


const char* CSearchPath::GetPathString() const
{
	return g_PathIDTable.String( m_Path );
}


const char* CSearchPath::GetPathIDString() const
{
