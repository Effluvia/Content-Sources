								...
								: vgui::MenuButton( parent, panelName, text )
{
	...
	//MODDD - casted filesystem() result to IBaseFileSystem.
...
void CGameMenuButton::OnOpenMultiplayerDialog()
{
	...
	{
		//MODDD - ?
		//MODDD - CRITICAL.  Should what's sent be 'this' or 'this->GetParent'?
		// I feel GetParent, but another codebase does 'this' instead.  Don't know.
		// seems weird to treat this button as the 'parent' of a new panel instead of whatever
		// parent this button has (which may or may not even be the whole window?  Could be some
		// smaller pane on the window instead?   Ehhhhh)
		//m_hMultiplayerCustomizeDialog = new CMultiplayerCustomizeDialog( );
...
void CGameMenuButton::OnOpenContentControlDialog()
{
	if ( !m_hContentControlDialog.Get() )
	{
		...
		m_hContentControlDialog = new CContentControlDialog(this);
