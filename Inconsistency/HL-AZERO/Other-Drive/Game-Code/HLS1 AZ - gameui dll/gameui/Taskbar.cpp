...
void CTaskbar::UpdateGameMenus()
{
	...
	const char *lvl = goldsrc_engine->pfnGetLevelName();

	if (lvl && *lvl)
	{
		isInGame = true;
		if (goldsrc_engine->GetMaxClients() > 1)
		{
			isMulti = true;
		}
	}
...
struct vgui::PanelMessageMap* CTaskbar::GetMessageMap(){
	return NULL;
}
/*
...
*/
void CTaskbar::OnCommand(char* command){
...
void CTaskbar::OnActivateModule(int moduleIndex){
	//ActivateModule(&g_VModuleLoader,moduleIndex);
	return;
}		
*/
...
{
	//MODDD - NOTE.  Found this way, and yes, even in goldsrc from what I can tell
	//!! tasks disabled
	return;
