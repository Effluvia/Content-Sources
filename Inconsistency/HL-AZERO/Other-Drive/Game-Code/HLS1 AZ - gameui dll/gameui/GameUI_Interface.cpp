...
//MODDD
// wait.  vgui/ILocalize.h is above.
// What the fuck.
// Oh silly me.  Not using namespace 'vgui'... put that in front of stuff
// from there.
...
// NOTICE!!! Many referneces to 'staticpanel' commented out, see the note in public/vgui_controls/Panel.h
// about how to stop crashes from referring to panel-related things (need to undo the changes there by
// restoring from a slightly earlier backup)
...
//MODDD - use 'goldsrc_engine' as a pointer to gEngfuncs instead to mimic the feel
// of 'engine' for the Source engine I guess?
cl_enginefunc_t* goldsrc_engine;
...
//LPDWORD ?
// or DWORD whatever
DWORD someGlobalShit = 0;

BOOL stopDaLoop = FALSE;
HANDLE threadhandle_render = NULL;
...
static void* __CreateCGameUIIGameUI_interface() {
	return (IGameUI *)&g_GameUI;
}
static InterfaceReg __g_CreateCGameUIIGameUI_reg(__CreateCGameUIIGameUI_interface, GAMEUI_INTERFACE_VERSION);
...
CGameUI::~CGameUI()
{
	...
	//return 0;
	//MODDD - WARNING!  Windows-specific.
	PostThreadMessage(someGlobalShit, WM_QUIT, (WPARAM) NULL, (LPARAM) NULL);
	WaitForSingleObject(threadhandle_render, 1000);
...
DWORD WINAPI myRenderThread(void* arg_data) {
	...
	while(!stopDaLoop){
		...
		vgui::VPANEL funnyMusic = vgui::surface()->GetEmbeddedPanel();
		// if the embedded panel isn't assigned yet, it will crash
		if(funnyMusic != NULL){
			...
			//BasePanel()->PaintBackground();
			//staticPanel->PaintBackground();
			//staticPanel->CBasePanel::letsMaybeNot();
		}

		// not INFINITE
		WaitForSingleObject(threadhandle_render, 50);
...
*/
	...
	CreateInterfaceFn breakPointerTemp[5];
	for(i = 0; i < count; i++){
		breakPointerTemp[i] = factories[i];
	}
	...
	*/
	...
	if(count >= 3){
		// woohoo (goldsource)
		// Don't see any unique 'clientFactory' being given?
		factory = factories[ 0 ];
		fileSystemFactory = factories[ 2 ];
		vguiFactory = factories[ 1 ];
		engineFactory = factories[ 0 ];
		clientFactory = factories[ 0 ];
	}else{
		// as-is way (source)?
		factory = factories[ 0 ];
		fileSystemFactory = factories[ 0 ];
		vguiFactory = factories[ 0 ];
		engineFactory = factories[ 0 ];
		clientFactory = factories[ 0 ];
	}
	/////////////////////////////////////////////////////////////////////
	...
	*/
	...
	if(safeCount > 4){
		// WARNING!!!   Something got cutoff from factories then (#0 is reserved for GameUI's own factory).
		safeCount = 4;
	}
	// one beyond, since safeCount counts the number of ones given by hl.exe (factories; 'count')
	int finalCount = safeCount + 1;

	int nexto = 2;
	for(i = 1; i < safeCount; i++){
		//MODDD - why not put these in the list??

		if(factories[i] == m_FactoryList[ 0 ] ){
			// already matches the one here in GameUI?  Skip it, don't need it
			// ('nexto' does not go up, that would leave an empty space)
			// reduce the count saved by 1 too.
			finalCount--;
			continue;
		}

		m_FactoryList[nexto] = factories[i];
		nexto++;
	}
	m_iNumFactories = finalCount;

	//m_FactoryList[2] = factories[1];
	//m_FactoryList[3] = factories[2];
	//m_FactoryList[4] = factories[3];



	//NOTE!!!  See VGUI_SURFACE_INTERFACE_VERSION.   Should this be "VGUI_Surface026" instead?

	//MODDD - PHUCC DAT.  Or maybe it's ok even if it doesn't do anything
	// from goldsrc calls?
	//MODDD - wait.    Why is a count of 2 being sent? Why not the 'count' from this method's own parameter list?
	// (now using the filtered finalCount)
	//vgui::VGui_InitInterfacesList( "GameUI", m_FactoryList, 2 );
	vgui::VGui_InitInterfacesList( "GameUI", m_FactoryList, finalCount );
	...
	*/
	...
	this->m_FactoryList[0] = Sys_GetFactoryThis();
	this->m_FactoryList[1] = factories[0];
	this->m_FactoryList[2] = factories[1];
	this->m_FactoryList[3] = factories[2];
	//this->m_FactoryList[4] = paVar5;
  
	this->m_FactoryList[4] = NULL;
	if (count > 5) {
		this->m_FactoryList[4] = factories[5];
	}
  
	this->m_iNumFactories = count;

	vgui::VGui_InitInterfacesList( "GameUI", m_FactoryList, count );
	...
	// TEST!  Will this return what we expect ot prove it called the right thing?
	// Did it even return INVALID_STRING_INDEX back then?  Who knows
	vgui::StringIndex_t testiii = vgui::localize()->FindIndex("doesnotexist");

	BOOL daMatch = (testiii == vgui::INVALID_STRING_INDEX);
...
void CGameUI::Start(struct cl_enginefuncs_s *engineFuncs, int interfaceVersion, IBaseSystem *system)
{
	...
	undefined4 uVar1;
	  ...
	  this->m_pMaster = (CSocket *)0x0;
	  */
	// ???  do we need to do this
	  strncpy(this->m_szPlatformDir,"platform",0x104);
...
//	m_pMaster = NULL;
	...
	//MODDD - ?????????   WHY NOT  (found commented out)
	memcpy(&gEngfuncs, engineFuncs, sizeof(gEngfuncs));
	// maybe not this, just use gEngfuncs whenever possible in its place.
	// Beware of what goldsrc had to do differnetly, maybe?  Like starting up
	// from GameUI.dll since TrackerUI wasn't a think in GoldSrc?
	//engine = &gEngfuncs;
	// use this as the pointer to the engine instead
	goldsrc_engine = &gEngfuncs;

	/*
	long eh = goldsrc_engine->pfnRandomLong(0, 12);
	long ey = goldsrc_engine->pfnRandomLong(0, 12);
	long ah = goldsrc_engine->pfnRandomLong(0, 12);
	*/
...
//	vgui::surface()->SetWorkspaceInsets( 0, 0, 0, g_pTaskbar->GetTall() );
	...
	// not working?     ok.     sure.
	// start mp3 playing
	//goldsrc_engine->pfnClientCmd("mp3 loop media/gamestartup.mp3\n");
	...
	//MODDD - NEW??
	//IVGuiModuleLoader* mytesto = (IVGuiModuleLoader * )engineFactory( VGUIMODULELOADER_INTERFACE_VERSION, NULL );
...
bool CGameUI::FindPlatformDirectory(char *platformDir, int bufferSize)
{
	...
	if (platformDir[0] == '\0')
	{
		...
		if (vgui::filesystem()->FileExists(pszServerBrowserDLL))
		{
			...
			vgui::filesystem()->GetLocalPath(pszServerBrowserDLL, szPlatformPath, MAX_PATH);
...
void CGameUI::OnDisconnectFromServer(int eSteamLoginFailure,char *username){
	// NEW from goldsrc.  ???
}
...
//	TRACE_FUNCTION("CGameUI::ActivateGameUI");
	...
	//MODDD - does goldsrc even need/want this?
	goldsrc_engine->pfnClientCmd("setpause");
	...
	//MODDD - WARNING!  Windows-specific.
	////////////////////////////////////////////////////////////////////////
	threadhandle_render = CreateThread(NULL, 0, &myRenderThread, NULL, 0, &someGlobalShit);
	if(threadhandle_render){
		// ?
	}
...
void CGameUI::RunFrame()
{
	...
	//MODDD - NO LOADING TRACKER FOR YOU
	/*
	if (m_bTryingToLoadTracker && g_hMutex && g_hWaitMutex)
	{
		...
		if (waitResult == SYS_WAIT_OBJECT_0 || waitResult == SYS_WAIT_ABANDONED)
		{
			...
			const char* fullGamePath = goldsrc_engine->pfnGetGameDirectory();
...
/*	if( m_pMaster )
	...
	*/
	...
	//MODDD - condition removed.
	// win32 surface doesn't even use this, why bother checking.
	// Shouldn't a detail like this be left up to the surface anyway?
