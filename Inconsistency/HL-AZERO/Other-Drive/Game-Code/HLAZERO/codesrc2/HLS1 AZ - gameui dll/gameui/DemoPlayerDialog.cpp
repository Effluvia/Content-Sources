...
CDemoPlayerDialog::CDemoPlayerDialog(vgui::Panel *parent) : Frame (parent, "DemoPlayerDialog")
{
	//MODDD - dummied
	/*
	m_World = NULL;
...
void CDemoPlayerDialog::Update()
{
	...
	char * timeScaleString = "";
	char * descrition = "Playing";
...
bool CDemoPlayerDialog::LoadModules()
{
	...
	m_Engine = (IEngineWrapper*) m_System->GetModule( INTERFACE_ENGINEWRAPPER_NAME, "", NULL );
	...
	m_DemoPlayer = (IDemoPlayer*) m_System->GetModule( INTERFACE_DEMOPLAYER_NAME, "", NULL );
...
void CDemoPlayerDialog::OnMessage(KeyValues *params, VPANEL fromPanel)
{
	...
	if ( !strcmp(params->GetName(), "DemoSelected") )
	{
		char fullstring[270];
		sprintf(fullstring, "viewdemo %s\n", params->GetString("demoname"));
				
		m_DemoPlayer->Stop();
		m_Engine->Cbuf_AddText( fullstring );
	}

	if ( !strcmp(params->GetName(), "ButtonToggled") )
	{
