...
void Menu::OnMenuItemSelected(Panel *panel)
{
	...
	if (item)
	{
		...
		if (parentMenu)
		{
			...
			ivgui()->PostMessage(parentMenu->GetVPanel(),  kv, GetVPanel(), 0);
		}
	}
	...
	if (GetParent())
	{
		...
		ivgui()->PostMessage(GetParent()->GetVPanel(), kv, GetVPanel(), 0);
	}
}
...
void Menu::OnKeyCodePressed(KeyCode code)
{
	...
	if (GetParent())
	{
		ivgui()->PostMessage(GetParent()->GetVPanel(), new KeyValues("KeyModeSet"), GetVPanel(), 0);
	}
}
